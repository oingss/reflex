//! Block（拦截/丢弃）出站。
//!
//! 拆分自原 `direct.rs`，对齐 sing-box `protocol/block/outbound.go`：
//! 拦截出站负责"主动断开/静默丢弃"流量，不再混入直连出站文件中。
//!
//! 行为：
//! - `method = "drop"`（或不区分大小写）：静默丢弃——读掉客户端发来的字节
//!   并丢弃，不主动关闭连接，让连接挂起直到客户端放弃；可让主动扫探/审查
//!   系统更难区分"被墙"和"目标不存在"。
//! - `method = "default"` 或不填：立即 drop 连接，等同于 sing-box 的
//!   `syscall.EPERM` 行为（直接拒绝）。
//!
