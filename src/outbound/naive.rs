//! NaiveProxy 出站实现
//!
//! 基于 sing-box 的 naive 协议，使用 HTTP/2 CONNECT 隧道。
//!
//! sing-box 原生使用 cronet-go（Chrome 网络栈）实现，Rust 生态无对应绑定，
//! 此实现使用 `h2` crate 提供 HTTP/2 CONNECT 隧道，协议层完全兼容：
//! - TLS 握手 → HTTP/2 协商
//! - CONNECT 请求携带 `Proxy-Authorization: Basic` 和 `Padding` 头
//! - 200 响应后双向流使用 8 帧 padding 分帧（与 sing-box naive inbound 互通）
//!
//! QUIC (HTTP/3) 模式字段已保留，当前不支持（返回错误）。
//!
//! ## 协议参考
//! - https://github.com/SagerNet/naiveproxy
//! - sing-box: protocol/naive/{outbound,inbound,inbound_conn}.go
...
use crate::{
    ...
    outbound::{
        anytls::{build_uot_packet, build_uot_request, read_uot_packet, UOT_MAGIC_ADDRESS,
                 UOT_MAGIC_PORT},
        apply_mark_to_tcp, relay, resolve_server_addr, set_tcp_opts,
        tls::build_client_config, Outbound,
    },
};
...
impl NaiveOutbound {
    ...
    async fn dial(&self, target: &Target) -> anyhow::Result<NaiveStream> {
        ...
        // 6. 发送 CONNECT 请求
        let mut h2_ready = send_req.ready().await.map_err(|e| {
            anyhow::anyhow!("naive: h2 ready failed: {e}")
        })?;
...
mod tests {
    ...
    fn padding_header_format() {
        ...
        for b in hdr.bytes().take(16) {
            assert!(PADDING_HEADER_CHARSET.contains(&b), "char {} not in charset", b);
        }
        // 其余为 ~
