...
pub async fn connect_tls_or_utls(
    ...
    // uTLS 分支
    //
    // 当前 uTLS 实现（utls.rs）通过拦截 rustls 的第一次 write 并替换为伪造的
    // ClientHello 来实现指纹伪装。但伪造 ClientHello 中的 key_share 使用了
    // **随机生成的** x25519 公钥，与 rustls 内部使用的私钥不匹配。
    //
    // 在 TLS 1.3 中，服务端用伪造公钥计算共享密钥，rustls 用自己的私钥计算，
    // 两者不一致 → Finished 消息 MAC 验证失败 → 握手中止。
    //
    // clash-rs 0.10.8 同样未实现 uTLS（client_fingerprint 字段被解析后直接忽略，
    // 仅打印警告），指纹伪装依赖 REALITY（在 rustls 内部实现，密钥交换正确）。
    //
    // 因此此处与 clash-rs 行为对齐：当 uTLS 启用时，打印警告并回退到标准 rustls
    // TLS。保留 utls 模块供未来正确实现使用。
    if let Some(utls_cfg) = &tls.utls {
        if utls_cfg.enabled {
            tracing::warn!(
                server_name = %server_name,
                fingerprint = ?utls_cfg.fingerprint,
                "uTLS fingerprinting is requested but not yet implemented; \
                 falling back to standard rustls TLS (same behavior as clash-rs). \
                 For fingerprint evasion, consider using REALITY instead."
            );
        }
    }
