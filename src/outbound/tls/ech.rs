//! 出站 ECH（Encrypted Client Hello，RFC 9460）支持
//!
//! ## 设计
//!
//! ECH 通过将真实的 inner ClientHello 用 HPKE 加密封装到 outer ClientHello
//! 的 `encrypted_client_hello`（扩展类型 `0xfe0d`）扩展中，从而隐藏 SNI 等
//! 敏感字段，防止中间人观察客户端正在连接的真实域名。
//!
//! 本模块参照 sing-box `common/tls/ech.go` / `ech_shared.go` 的设计，提供：
//!
//! - [`parse_ech_config_pem`]：解析 PEM `ECH CONFIGS` 块，返回原始 ECHConfigList 字节
//! - [`EchConfig`] / [`parse_ech_config_list`]：解析 ECHConfigList 二进制结构（RFC 9460 §4）
//! - [`resolve_ech_config_list`]：从 [`OutboundECHOptions`] 解析出 ECHConfigList
//!   （优先 `config` → `config_path`，与 sing-box `parseECHClientConfig` 一致）
//! - [`fetch_ech_config_from_dns`]：通过 DNS HTTPS RR（type 65）的 `ech` SVCB
//!   参数获取 ECHConfigList（对应 sing-box `ECHClientConfig.fetchAndHandshake`）
//! - [`connect_ech`]：ECH 握手入口
//!
//! ## 当前限制
//!
//! rustls 0.23 尚未原生支持 ECH。由于 ECH 要求 inner/outer ClientHello 分裂、
//! HPKE 密封以及 ECH 接受确认验证，无法通过简单的 ClientHello patch 在 rustls
//! 之上实现——必须完整自实现 TLS 1.3 握手（类似 `reality.rs`）。
//!
//! 当前 [`connect_ech`] 在配置和 ECHConfigList 解析完成后返回明确错误，
//! 等 rustls 原生支持 ECH（或引入自定义 TLS 1.3 客户端）后即可直接接入。
...
pub fn parse_ech_config_pem(pem_text: &str) -> anyhow::Result<Vec<u8>> {
    ...
    if pem_text[body_start + end_pos + end.len()..]
...
pub fn resolve_ech_config_list(
    options: &OutboundECHOptions,
