//! REALITY 客户端握手实现（基于 meow-rs 的纯 Rust TLS 1.3 方案）
//!
//! 旧方案使用 rustls + PrependedTcpStream 拦截/patch ClientHello，但存在多个问题：
//! - rustls 内部状态与 wire 上的 ClientHello 不一致
//! - 自定义 KxGroup 注入不可靠
//! - 服务端 REALITY 验证失败返回 "processed invalid connection"
//!
//! 新方案（参考 meow-rs reality_tls.rs）：完全自实现 TLS 1.3 握手，不依赖 rustls。
//! 只实现 REALITY 所需的最小 TLS 1.3 子集：
//! - 手动构造含 REALITY 认证标记的 ClientHello
//! - 解析 ServerHello，提取 key_share 做 ECDHE
//! - 派生 handshake/application keys
//! - 解密服务端 EncryptedExtensions/Certificate/CertificateVerify/Finished
//! - 验证 REALITY 证书（HMAC-SHA512）
//! - 发送 client Finished
//! - 后续使用 application keys 加解密数据
...
async fn read_plain_handshake<R: AsyncRead + Unpin>(r: &mut R, expected: u8) -> anyhow::Result<Vec<u8>> {
    loop {
        let record = read_record(r).await?.ok_or_else(|| {
            anyhow::anyhow!("REALITY: EOF while reading ServerHello")
        })?;
        debug!(
...
async fn fill_decrypted_handshake<R: AsyncRead + Unpin>(
    ...
    loop {
        let record = read_record(r).await?.ok_or_else(|| {
            anyhow::anyhow!("REALITY: EOF during encrypted handshake")
        })?;
        if record.typ == TLS_RECORD_CHANGE_CIPHER_SPEC {
...
fn parse_server_hello(raw: &[u8]) -> anyhow::Result<ParsedServerHello> {
    ...
    Ok(ParsedServerHello {
        ...
        session_id,
        key_share: key_share
            .ok_or_else(|| anyhow::anyhow!("REALITY: missing X25519 key share"))?,
    })
}
...
impl RecordKey {
    ...
    fn encrypt_detached(&self, nonce: &[u8; 12], aad: &[u8], body: &mut [u8]) -> anyhow::Result<Tag> {
        match &self.cipher {
