...
impl VlessOutbound {
    ...
    fn build_tls_config(tls: &crate::config::outbound::VlessTlsConfig) -> crate::config::outbound::TlsConfig {
    ...
    fn build_request_header(uuid: &[u8; 16], target: &Target, flow: Option<&str>) -> anyhow::Result<BytesMut> {
...
impl Outbound for VlessOutbound {
    ...
    async fn handle_udp(&self, mut packet: InboundUdpPacket) -> anyhow::Result<()> {
        ...
        let first_dst_addr = resolve_target_with_dns(&packet.target, self.resolver.as_ref()).await?;
