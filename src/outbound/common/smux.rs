//! SMux 多路复用（Multiplex）实现
//!
//! 协议兼容 sing-box / clash-meta 使用的 smux v1/v2 格式。
//!
//! ## 帧格式（SMux v1）
//!
//! ```text
//! +-------+---------+--------+--------+
//! | Ver   | Cmd     | Length | Stream |
//! | 1B    | 1B      | 2B LE  | 4B LE  |
//! +-------+---------+--------+--------+
//! | Data (Length bytes)               |
//! +-----------------------------------+
...
//! ## SMux v2 额外字段
//!
//! v2 在 Data 之前插入一个 32 位 **消费窗口** 字段（仅 cmdPSH 帧）：
//! ```text
//! +-------+---------+--------+--------+
//! | Ver   | Cmd     | Length | Stream |
//! +---+---+---------+--------+--------+
//! | Consumed (4B LE, v2 cmdPSH only)  |
//! +-----------------------------------+
//! | Data                              |
//! +-----------------------------------+
//! ```
...
//! ```rust,ignore
//! // 建立 TCP 连接后
//! let session = SmuxSession::new(tcp_stream, SmuxConfig::default());
//! // 打开一个新的逻辑流
//! let stream = session.open_stream().await?;
//! // 像普通 AsyncRead/AsyncWrite 使用 stream
//! ```
...
impl From<&MultiplexConfig> for SmuxConfig {
    fn from(mc: &MultiplexConfig) -> Self {
        let version = if mc.protocol.starts_with("smux") || mc.protocol.starts_with("yamux") {
            SMUX_V1 // yamux 降级到 smux v1 帧格式（大多数场景兼容）
        } else {
            SMUX_V1
        };
        Self {
