//! Tailscale 出站实现（参照 clash-rs `proxy/tailscale`，基于 `tailscale-rs` crate）。
//!
//! 通过 Tailscale userspace netstack（`tailscale::Device`）建立到 Tailscale
//! 网络中其他节点或经由 subnet router 可达的内部服务的连接。
//!
//! # 工作原理
//!
//! 1. **首次连接时**：根据 `auth_key` / `hostname` / `control_url` 初始化
//!    一个 `tailscale::Device`（userspace netstack + DERP + WireGuard），
//!    缓存在 `tokio::sync::Mutex` 内部，后续连接复用同一个 Device。
//! 2. **TCP 转发**：调用 `Device::tcp_connect((ip, port))` 在 Tailscale 网络
//!    内建立 TCP 连接，由 smoltcp netstack 承载。
//! 3. **UDP 转发**：调用 `Device::udp_bind((ip, port))` 绑定一个 UDP socket，
//!    再通过 `send_to` / `recv_from` 收发数据报。
//!
//! # 路由说明
//!
//! 与 clash-rs 一致：**不暴露 exit-node 选择**。流量路由依赖 Tailscale 控制台
//! 中配置的 subnet router 通告的 CIDR。要访问某内部服务，把目标 IP 设为
//! 该 subnet 内的地址即可（路由表会通过 netmap 自动安装到 smoltcp）。
//!
//! # 限制
//!
//! - 域名目标必须先解析为 IP（Tailscale netstack 不走系统 DNS）
//! - `TS_RS_EXPERIMENT` 环境变量会在 `Device::new` 之前自动设置
//!   （上游 crate 强制要求确认实验性软件）
...
impl TailscaleOutbound {
    ...
    async fn get_device(&self) -> io::Result<Arc<tailscale::Device>> {
        ...
        let key_state = if self.config.ephemeral {
            ...
            let state_file =
                std::path::PathBuf::from(state_dir).join(TAILSCALE_STATE_FILE_NAME);
        ...
        if let Some(control_url) = self.config.control_url.as_ref() {
            ts_config.control_server_url = control_url
                .parse()
                .map_err(|e| io::Error::new(io::ErrorKind::InvalidInput, format!("invalid control_url: {e}")))?;
        }

        let device = tailscale::Device::new(&ts_config, self.config.auth_key.clone())
            .await
            .map_err(|e| {
                io::Error::other(format!("failed to initialize tailscale device: {e}"))
            })?;
        ...
        match target {
            ...
            Target::Domain(host, _) => {
                let ip = resolve_target_with_dns(target, self.resolver.as_ref())
                    .await
                    .map_err(|e| {
                        io::Error::other(format!("dns resolve '{host}' failed: {e}"))
                    })?;
                Ok(ip.ip())
            }
        }
...
impl Outbound for TailscaleOutbound {
    ...
    async fn handle_tcp(&self, conn: InboundTcpStream) -> anyhow::Result<(u64, u64)> {
        ...
        let stream = device
            ...
            .map_err(|e| {
                anyhow::anyhow!(
                    "tailscale: tcp_connect to {ip}:{port} failed: {e}"
                )
            })?;
    ...
    async fn handle_udp(&self, mut packet: InboundUdpPacket) -> anyhow::Result<()> {
        ...
        let udp = device
            ...
            .await
            .map_err(|e| anyhow::anyhow!("tailscale: udp_bind on {local_ip}:{local_port} failed: {e}"))?;
            ...
            tokio::spawn(async move {
                while let Some((target, data)) = upstream_rx.recv().await {
                    ...
                    let dst = match target {
                        Target::Socket(a) => a,
                        Target::Domain(_, _) => match resolve_target_with_dns(&target, None).await {
                            Ok(a) => a,
                            Err(_) => continue,
                        },
                    };
