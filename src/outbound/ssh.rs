//! SSH 出站实现（参照 clash-rs `proxy/ssh`，基于 `russh` crate）。
//!
//! 通过 SSH 协议建立到远端服务器的 SSH 隧道，使用 `direct-tcpip` channel
//! 转发 TCP 流量到目标地址。
//!
//! # 支持的认证方式
//!
//! 按以下顺序尝试，直到服务器接受为止：
//! 1. **公钥认证**（`private_key`）：使用本地私钥进行公钥认证
//! 2. **密码认证**（`password`）：使用密码进行认证
//! 3. **键盘交互认证**：用于支持服务端要求的二次验证（如 TOTP 2FA），
//!    当 prompt 为 `"Verification code: "` 时使用 `totp_opt` 配置的 TOTP 生成当前验证码，
//!    当 prompt 为 `"Password: "` 时回退使用 `password`
//!
//! # 限制
//!
//! - 仅支持 TCP 流量转发（SSH UDP 转发尚未实现）
//! - 不支持 SSH 端口转发之外的 SSH 功能（如 SFTP、shell）
//!
//! # 协议参考
//!
//! - RFC 4254 §6.2: direct-tcpip channel
...
impl SshOutbound {
        ...
        match opt {
            TotpOption::OtpAuth { secret } => {
                let rfc6238 = totp_rs::Rfc6238::with_defaults(
                    totp_rs::Secret::Encoded(secret.clone()).to_bytes().map_err(|e| {
            ...
            match resp {
                ...
                KeyboardInteractiveAuthResponse::InfoRequest { prompts, .. } => {
                    ...
                    resp = session
                        ...
                        .await
                        .map_err(|e| anyhow::anyhow!("ssh: keyboard-interactive respond failed: {e}"))?;
                }
            }
        }
    ...
    async fn open_channel(
        &self,
        target: &Target,
    ) -> anyhow::Result<SshChannelStream> {
