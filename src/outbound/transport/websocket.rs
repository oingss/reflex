    ...
    S: Sink<Message, Error = tokio_tungstenite::tungstenite::Error> + Unpin,
{
    fn poll_write(
        ...
        // 重试时只发裸数据 → 协议帧错位。
        //
        // 性能说明（与 sing-box vless client.go:149-158 的 ExtendHeader 对齐）：
        // - 首帧（带 header）：用 `BytesMut::with_capacity(header.len()+data.len())`
        //   一次性预分配，header + data 拼成一个连续 buffer 后 freeze 成 Bytes，
        //   再 into Vec。整个路径只有 1 次堆分配 + 1 次 memcpy（freeze 后 into Vec
        //   是零拷贝的指针转移）。
        // - 后续帧（无 header）：tungstenite 0.24 的 `Message::Binary` 拥有 `Vec<u8>`
        //   所有权，调用者借出的 `&[u8]` 必须拷一次。这是 tungstenite API 的硬约束，
        //   除非升级到支持 `Message::Binary(Bytes)` 的版本。当前路径与 sing-box
        //   `bufio.NewVectorisedWriter` 的「直接 writev 到底层 conn」相比多一次
        //   拷贝，但已无法在 0.24 API 下消除。
        let (payload, header_consumed) = if let Some(header) = this.pending_header.as_ref() {
            ...
            // Bytes → Vec 是零拷贝的指针转移（Bytes 内部已是 Box<[u8]>）。
...
///
/// 与 `crate::protocol::vless::parse_response` 语义一致，
/// 此处内联以避免传输层对协议帧模块的依赖。
...
mod tests {
    ...
    async fn vless_ws_roundtrip() {
        ...
        let (ws_stream, _resp) = tokio_tungstenite::client_async("ws://localhost/test", client_io)
            .await
            .expect("ws connect");
    ...
    async fn vless_ws_tls_roundtrip() {
        ...
        // ── Client: TCP → TLS (standard rustls) → WS → VLESS header merge ────
        let tcp = crate::outbound::connect_tcp_interface(addr)
            .await
            .expect("connect");

        // TLS config with standard rustls (no uTLS).
        // 注：旧测试启用 uTLS 测试「fallback 到 rustls」行为，但 key_share patching
        // 修复后 uTLS 不再 fallback——会真正发送伪造 Chrome ClientHello。自签证书的
        // rustls 测试服务器无法完成伪造 ClientHello 的握手，故此处改为标准 rustls。
        // uTLS 的正确性需独立集成测试（需真实支持 Chrome 指纹的服务端）。
        ...
        let tls_stream = crate::outbound::tls::connect_tls_or_utls(tcp, "localhost", &tls_config)
            .await
            .expect("tls connect (utls fallback)");

        let (ws_stream, _resp) = tokio_tungstenite::client_async("ws://localhost/test", tls_stream)
            .await
            .expect("ws connect");
