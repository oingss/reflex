...
//! gRPC 传输模式。参照 clash-rs `proxy/transport/grpc.rs` 与 Xray-core
//! `transport/internet/grpc` 实现。
//!
//! # 工作原理
//!
//! 客户端与服务器建立 HTTP/2 连接（先建立 TCP，可选 TLS，再在其上握手 h2），
//! 随后向 `{path}/Tun` 发送 POST 请求，content-type 为 `application/grpc`。
//! 数据按 gRPC 帧格式封装：
//!
//! ```text
//! [压缩标志 1B][长度 4B BE][protobuf field tag 0x0a][varint 长度][data]
//! ```
//!
//! 服务端将响应数据以相同格式回送。本模块将 gRPC 双向流适配为
//! `AsyncRead + AsyncWrite`，供上层协议透明使用。
//!
//! # TLS 路径
//!
//! 与 sing-box / WebSocket 传输一致：gRPC over TLS 时先通过
//! [`crate::outbound::tls::connect_tls_or_utls`] 建立 TLS 流（支持 uTLS），
//! 再在 TLS 流上进行 h2 握手。ALPN 强制为 `h2`（gRPC 必须基于 HTTP/2）。
