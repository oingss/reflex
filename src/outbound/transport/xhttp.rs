...
pub async fn connect(
    ...
    debug!(
        ...
        insecure = tls.map_or(false, |t| t.insecure),
    ...
    fn call(&mut self, uri: Uri) -> Self::Future {
        ...
        Box::pin(async move {
            ...
            if let Some(tls) = tls_cfg {
                ...
                debug!(sni = sni_str, uri_host = host, "xhttp connector: starting TLS handshake");
            ...
            while let Some(chunk) = up_rx.recv().await {
                ...
                while !remaining.is_empty() {
                    let payload: Bytes = if remaining.len() > shared.max_post_bytes {
                        ...
                        std::mem::take(&mut remaining).into()
                    };
                ...
                match stream.frame().await {
                    None => {
                        debug!(
                            total_bytes,
                            frame_count,
                            "xhttp download: stream ended (no more frames)"
                        );
                        break;
                    }
                    Some(Ok(frame)) => {
                        if let Ok(data) = frame.into_data() {
                            ...
                            if tx.send(Ok(data)).await.is_err() {
                                debug!(
                                    total_bytes,
                                    frame_count,
                                    "xhttp download: consumer dropped, stopping"
                                );
                                break;
                            }
                        }
