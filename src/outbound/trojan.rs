...
/// Trojan UDP 帧最大载荷（防 DoS）。与 sing-box 协议层对齐：
/// 单个 UDP 包在 Trojan 隧道中不应超过标准 IPv4 最大 UDP 负载（65535 字节）。
        ...
        loop {
            ...
            let (_consumed, _target) = match atyp_buf[0] {
                ATYP_IPV4 => {
                    ...
                    read_socks_addr(&addr_buf[..7])?
                }
                ATYP_IPV6 => {
                    reader.read_exact(&mut addr_buf[1..19]).await?;
                    read_socks_addr(&addr_buf[..19])?
                }
                ATYP_DOMAIN => {
                    ...
                    read_socks_addr(&addr_buf[..total])?
                }
                _ => break,
            };
            ...
            if data_len > MAX_UDP_PAYLOAD {
                return Err(anyhow::anyhow!(
                    "trojan udp: payload too large ({data_len} > {MAX_UDP_PAYLOAD})"
                ));
            }
...
///
/// 在 Trojan UDP 接收路径中用于精确解析变长地址，替代旧实现的硬编码偏移
/// 跳过（无法正确处理 Domain 地址的长度变化）。
fn read_socks_addr(data: &[u8]) -> anyhow::Result<(usize, Target)> {
