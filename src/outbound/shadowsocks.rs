...
use crate::{
    ...
    outbound::{
        apply_mark_to_tcp, apply_mark_to_udp, set_tcp_opts, Outbound,
        OutboundStatus,
    },
};
...
impl ShadowsocksOutbound {
    ...
    async fn connect_ss_xhttp(
        ...
        let subkey = self.derive_subkey(&salt);
        Ok(Box::new(
            ss_wrap_xhttp(stream, self.method, subkey, salt, first_payload, self.key_material.clone()).await?,
        ))
    }
    ...
    async fn connect_ss_ws(
        ...
        let subkey = self.derive_subkey(&salt);
        Ok(Box::new(
            ss_wrap_xhttp(ws_io, self.method, subkey, salt, first_payload, self.key_material.clone()).await?,
        ))
    }
    ...
    async fn connect_ss_grpc(
        ...
        let subkey = self.derive_subkey(&salt);
        Ok(Box::new(
            ss_wrap_xhttp(stream, self.method, subkey, salt, first_payload, self.key_material.clone())
                .await?,
        ))
    }
}
...
impl Outbound for ShadowsocksOutbound {
    ...
    async fn handle_tcp(&self, conn: InboundTcpStream) -> anyhow::Result<(u64, u64)> {
        ...
        if let Some(ref pool) = self.mux_pool {
            ...
            let subkey = self.derive_subkey(&salt);
            let ss_stream =
                ss_wrap_xhttp(mux_stream, self.method, subkey, salt, first_payload, self.key_material.clone()).await?;
                ...
                while let Some((target, data)) = upstream_rx.recv().await {
                    ...
                    let wire = if method == Method::None {
                        ...
                        let now = std::time::SystemTime::now()
                            ...
                            .as_secs();
                        let mut body =
                            ss2022_udp_build_client_body(now, &socks_addr, &data);
