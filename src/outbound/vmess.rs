...
//! # 模块组织
//!
//! 本文件由原 `vmess/` 目录下的三个文件合并而成：
//! - 协议帧构建与 KDF 派生（原 `frame.rs`）
//! - AEAD 数据传输层分帧读写器（原 `aead.rs`）
    ...
    async fn connect_raw(&self) -> anyhow::Result<Box<dyn AsyncReadWrite>> {
        match &self.config.transport {
            VmessTransportConfig::Ws(ws_cfg) => {
                ...
                .await?;
                Ok(Box::new(crate::outbound::transport::websocket::WsStream::new(
                    ws,
                )))
            }
    ...
    pub(crate) async fn read_packet(&mut self, out: &mut [u8]) -> io::Result<usize> {
        ...
        if n < header_len {
            return Err(io::Error::new(
                io::ErrorKind::InvalidData,
                format!(
                    "vmess packetaddr: frame {n} bytes shorter than header {header_len}"
                ),
            ));
        }
