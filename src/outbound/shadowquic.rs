//! ShadowQuic 出站。
//!
//! 基于 shadowquic crate 实现 0-RTT QUIC + JLS SNI 伪装代理。
//! 参考 clash-rs `proxy/shadowquic/mod.rs` 的封装方式，对接 shadowquic crate v0.3.x API。
//!
//! ## 协议概述
//! - 0-RTT QUIC：首包即数据，降低握手延迟
//! - JLS：SNI 伪装，TLS 握手呈现的是伪装域名，实际连接 shadowquic 服务端
//! - TCP：通过 bi-stream 发送 SQConnect 请求，双向透传
//! - UDP：通过 associate_udp 获取 (Sender, Receiver)，桥接到 reflex UdpSession
//!   - over_stream=false：UDP over QUIC datagram（高效，推荐）
//!   - over_stream=true：UDP over QUIC uni-stream（兼容性更好）
//!
//! ## 连接池
//! shadowquic 的 `ShadowQuicClient::get_conn()` 每次调用都创建新的 QUIC 连接，
//! 因此在适配层自行维护 `Mutex<Option<ShadowQuicConn>>` 连接池：
//! - 优先复用未关闭的缓存连接（`close_reason().is_none()`）
//! - 连接已关闭时调用 `get_conn()` 重建
//! - `ShadowQuicClient` 本身存于 `OnceCell`，endpoint 只创建一次
