...
use crate::{
    ...
    protocol::shadowquic::target_to_socks_addr,
};
...
// `target_to_socks_addr` 已上移至 `crate::protocol::shadowquic`（与入站共享），
// 见文件头部的 use 导入。
...
let target = crate::inbound::Target::Domain("example.com".into(), 443);
...
let target = crate::inbound::Target::Socket("1.2.3.4:80".parse().unwrap());
        let addr = target_to_socks_addr(&target);
