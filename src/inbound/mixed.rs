        ...
        loop {
            ...
            tokio::spawn(async move {
                if let Err(e) = handle_conn(stream, peer, config, tcp_tx, udp_tx, tag).await {
                    debug!(peer = %display_sockaddr(peer), err = %e, "mixed inbound conn error");
                }
            });
        }
    }
...
async fn handle_socks5(
    ...
    match cmd {
        CMD_CONNECT => {
            ...
            debug!(peer = %display_sockaddr(peer), target = %target, "socks5 CONNECT");
...
async fn handle_socks5_udp_associate(
    ...
    debug!(peer = %display_sockaddr(peer), udp_port = %udp_local.port(), "socks5 UDP ASSOCIATE");
    ...
    udp_task.abort();
    reply_task.abort();
    debug!(peer = %display_sockaddr(peer), "socks5 UDP ASSOCIATE ended");
    Ok(())
}
...
async fn handle_http(
    ...
    debug!(peer = %display_sockaddr(peer), target = %target, "http CONNECT");
