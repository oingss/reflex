...
/// DNS 查询来源类型：用于区分查询是从专用 DNS 入站进入，还是被路由层
/// 劫持而来（端口 53 的 TCP/UDP 流量命中 hijack_dns 规则或全局开关）。
///
/// 该信息主要用于：
/// - 统计与日志：分别统计主动查询和被动劫持的 DNS 流量
/// - 调试：在 Clash API `/dns/query` 等接口中展示来源
/// - 未来扩展：劫持流量可能应用不同的 DNS 规则集
