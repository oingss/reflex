//! HTTP 代理入站：支持 HTTP CONNECT 隧道 + 普通转发代理。
//!
//! 认证方式：HTTP `Proxy-Authorization: Basic` 头（可选）。
//!
//! 配置写法与 sing-box 一致：
//! ```json
        ...
        loop {
            ...
            tokio::spawn(async move {
                if let Err(e) = handle_http(stream, peer, config, tcp_tx, tag).await {
                    debug!(peer = %peer, err = %e, "http inbound conn error");
                }
            });
        }
    }
...
async fn handle_http(
    ...
    debug!(peer = %peer, target = %target, connect = is_connect, "http request");
...
fn rewrite_request_for_forward(request: &str, buf: &[u8], header_end: usize) -> bytes::Bytes {
    ...
    let after_scheme = target.split_once("://").map(|(_, after)| after).unwrap_or("");
