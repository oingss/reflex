//! 网络接口监控。
//!
//! Linux: netlink 异步监听链路事件（RTMGRP_LINK | RTMGRP_IPV4_IFADDR）
//! macOS: 基于 kqueue 或轮询 /proc（不适用）→ sysctl/netlink 不可用，轮询 ifconfig
//! Windows: 基于 NotifyIpInterfaceChange WinAPI
//!
//! 参照 sing-tun InterfaceMonitor 的设计，在接口 UP/DOWN、地址变更时
//! 通知上层自动调整路由规则。
//! 当前为预留实现，尚未被主 TUN 流程引用。
...
fn check_android_vpn_active() -> bool {
    ...
    match out {
        Some(o) if o.status.success() => {
            ...
            stdout.lines().any(|line| line.contains("fwmark") && line.contains("0x20000"))
        }
        _ => false,
    }
}
...
async fn monitor_task() {
    ...
    loop {
        ...
        let changed = if last_events.len() != current.len() {
            ...
            last_events.iter().zip(current.iter()).any(|(a, b)| {
                a.up != b.up || a.name != b.name || a.mtu != b.mtu
            })
        };
