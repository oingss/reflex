//! TUN gvisor / mixed 协议栈适配层。
//!
//! 基于 netstack 模块（smoltcp 用户态协议栈），为 TUN inbound 提供
//! `gvisor` 栈（TCP + UDP 都走用户态协议栈）和 `mixed` 栈（TCP 走 system NAT，
//! UDP 走 gvisor）。
//!
//! 设计要点（参照 clash-rs `clash-lib/src/proxy/tun/runner.rs` 的四 task 编排）：
//!
//! 1. **TUN ↔ 协议栈 I/O**：把 `tun::AsyncDevice` 的读/write 适配为
//!    `futures::Sink<Packet>` / `futures::Stream<Item = Packet>`，与
//!    `NetStack::split()` 的 sink/stream 对接。
//! 2. **TCP dispatch**：`TcpListener` 返回的 `netstack::TcpStream` 实现
//!    `AsyncRead + AsyncWrite`。但 reflex 的 `SniffedStream`/`Outbound::handle_tcp`
//!    强绑定 `tokio::net::TcpStream`（涉及 15+ outbound 实现 + dispatcher）。
//!    为避免对核心抽象做泛型化重构，此处用 `tokio::net::TcpListener` 绑定
//!    `127.0.0.1:0` 建一条本地 OS socket，把 netstack 流双向 copy 到 OS socket，
//!    产出真实 `TcpStream` 交给 `SniffedStream::new`。
//! 3. **UDP dispatch**：`UdpSocket::split()` 返回 `(SplitRead, SplitWrite)`，
//!    `SplitRead::recv()` 返回 `UdpPacket { data, local_addr, remote_addr }`。
//!    映射到 reflex 的 `InboundUdpPacket { data, src, target, session, ... }`，
//!    并起一个回包 task 从 `UdpSession::reply_tx` 接收出站数据，调
//!    `SplitWrite::send()` 写回 TUN。
        ...
        while let Some(pkt) = udp_read.recv().await {
            ...
            let entry = sessions.entry(key).or_insert_with(|| {
                ...
                let (reply_tx, mut reply_rx) =
                    mpsc::channel::<(Bytes, SocketAddr, SocketAddr)>(64);
                ...
                tokio::spawn(async move {
                    while let Some((payload, _client_src, server_src)) =
                        reply_rx.recv().await
                    {
                        // 回包：IP src = 服务器(server_src)，IP dst = 客户端(src)。
                        if let Some(pkt) = super::build_udp_reply_packet(
                            server_src, src, &payload,
                        ) {
...
pub async fn run_mixed(
    ...
    use super::{
        handle_icmpv4, handle_icmpv6, handle_tcp_v4, handle_tcp_v6, TcpNat, DEFAULT_UDP_TIMEOUT_SECS,
        IPPROTO_ICMP, IPPROTO_ICMPV6, IPPROTO_TCP, IPPROTO_UDP, IPV4_VERSION, IPV6_VERSION,
    };
        ...
        Arc::new(Mutex::new(HashMap::new()));
    {
        let sessions = udp_sessions.clone();
        tokio::spawn(async move {
            let mut ticker = tokio::time::interval(Duration::from_secs(DEFAULT_UDP_TIMEOUT_SECS) / 2);
            loop {
                ticker.tick().await;
                sessions
                    .lock()
                    .await
                    .retain(|_, v| v.last_seen.elapsed() < Duration::from_secs(DEFAULT_UDP_TIMEOUT_SECS));
            }
        });
    }
    ...
    loop {
        ...
        match version {
            IPV4_VERSION => {
                ...
                let src_ip = Ipv4Addr::from([pkt_slice[12], pkt_slice[13], pkt_slice[14], pkt_slice[15]]);
                let dst_ip = Ipv4Addr::from([pkt_slice[16], pkt_slice[17], pkt_slice[18], pkt_slice[19]]);
                let payload = &pkt_slice[ihl..];
                match pkt_slice[9] {
                    IPPROTO_TCP => {
                        // TCP → system NAT（回包直接写 TUN）
                        handle_tcp_v4(
                            pkt_slice, payload, src_ip, dst_ip,
                            inet4_server_addr, inet4_client_addr,
                            inet4_loopback, tcp_port_v4, tcp_mss,
                            tun_writer.clone(),
                            tcp_nat.clone(),
                        ).await;
                    }
                    ...
                    IPPROTO_ICMP => {
                        handle_icmpv4(
                            pkt_slice, ihl, src_ip, dst_ip, inet4_server_addr,
                            tun_writer.clone(),
                        ).await;
                    }
                    _ => {}
            ...
            IPV6_VERSION => {
                ...
                match pkt_slice[6] {
                    IPPROTO_TCP => {
                        handle_tcp_v6(
                            pkt_slice, payload, src_ip, dst_ip,
                            inet6_server_addr, inet6_client_addr,
                            inet6_loopback, tcp_port_v6, tcp_mss,
                            tun_writer.clone(),
                            tcp_nat.clone(),
                        ).await;
                    }
                    ...
                    IPPROTO_ICMPV6 => {
                        handle_icmpv6(
                            pkt_slice, src_ip, dst_ip, inet6_server_addr,
                            tun_writer.clone(),
                        ).await;
                    }
                    _ => {}
