...
mod win32_route {
    ...
    use ::windows::Win32::Networking::WinSock::{
        IpPrefixOriginManual, IpSuffixOriginManual, AF_INET, AF_INET6, SOCKADDR_INET,
    };
    ...
    /// 删除 IPv4 路由。
    pub fn delete_route_v4(destination: Ipv4Addr, prefix_len: u8) -> std::io::Result<()> {
        let mut row = MIB_IPFORWARD_ROW2::default();
        unsafe { InitializeIpForwardEntry(&mut row) };
    ...
    /// 删除 IPv6 路由。
    pub fn delete_route_v6(destination: Ipv6Addr, prefix_len: u8) -> std::io::Result<()> {
        let mut row = MIB_IPFORWARD_ROW2::default();
        unsafe { InitializeIpForwardEntry(&mut row) };
    ...
    pub fn set_interface_dns(if_index: u32, servers: &[Ipv4Addr]) -> anyhow::Result<()> {
        ...
        if let Some(exe) = current_exe_path() {
            Command::new("netsh")
                .args([
                    ...
                    "add",
                    "rule",
                    "name=reflex-tun-strict-allow-v4",
                    "dir=out",
                    "action=allow",
                    "remoteip=0.0.0.0/0",
                    &format!("program={exe}"),
            ...
            Command::new("netsh")
                .args([
                    ...
                    "add",
                    "rule",
                    "name=reflex-tun-strict-allow-v6",
                    "dir=out",
                    "action=allow",
                    "remoteip=::/0",
                    &format!("program={exe}"),
        ...
        if !has_v4 {
            Command::new("netsh")
                .args([
                    ...
                    "add",
                    "rule",
                    "name=reflex-tun-strict-block-v4",
                    "dir=out",
                    "action=block",
                    "remoteip=0.0.0.0/0",
                ])
            ...
            if !remoteip.is_empty() {
                if let Some(exe) = current_exe_path() {
                    Command::new("netsh")
                        .args([
                            ...
                            "add",
                            "rule",
                            "name=reflex-tun-strict-allow-tun-v4",
                            "dir=out",
                            "action=allow",
                ...
                Command::new("netsh")
                    .args([
                        ...
                        "add",
                        "rule",
                        "name=reflex-tun-strict-block-tun-v4",
                        "dir=out",
                        "action=block",
                        &format!("remoteip={remoteip}"),
                    ])
