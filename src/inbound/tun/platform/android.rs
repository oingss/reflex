//! Android TUN auto_route / strict_route 实现。
//!
//! Android 使用 Linux 内核，但路由规则有显著差异：
//! - TUN 设备路径为 `/dev/tun`（而非 `/dev/net/tun`）
//! - 无 suppres_prefixlength 规则、无 dport 53 DNS 劫持
//! - 通过 0x20000 firewall mark 检测/旁路系统 VPN
//! - 包名 → UID 转换（解析 `/data/system/packages.xml`）
//! - 自动重定向需通过 `su` 执行 iptables
//! - 无网关地址（由 Android VPN 框架处理）
...
async fn build_android_uid_exclusions(
    ...
    let mut include_ranges: Vec<(u32, u32)> = include_users.iter().map(|&u| {
        (u * ANDROID_USER_RANGE, (u + 1) * ANDROID_USER_RANGE - 1)
    }).collect();
...
fn complement_ranges(ranges: &[(u32, u32)], start: u32, end: u32) -> Vec<(u32, u32)> {
    if ranges.is_empty() { return vec![(start, end)]; }
...
pub async fn setup(cfg: &TunInboundConfig, if_name: &str) -> anyhow::Result<SetupState> {
    ...
    // 确保设备 UP
    Command::new("ip").args(["link", "set", if_name, "up"])
        .output().ok();
    ...
    for (lo, hi) in &excluded_uids {
        if has_v4 {
            Command::new("ip")
                .args(["rule", "add", "priority", &prio_base.to_string(),
                    "uidrange", &format!("{lo}-{hi}"), "goto", &nop_prio.to_string()])
                .output().ok();
    ...
    if !cfg.include_interface.is_empty() {
        ...
        for iface in &cfg.exclude_interface {
            if has_v4 {
                Command::new("ip")
                    .args(["rule", "add", "priority", &prio_base.to_string(),
                        "iif", iface, "goto", &nop_prio.to_string()])
                    .output().ok();
                state.rule_priorities.push(prio_base);
            }
        }
    ...
    if vpn_enabled {
        ...
        if has_v4 {
            ...
            rule.args(["rule", "add", "priority", &prio_base.to_string(),
                "fwmark", &format!("{PROTECTED_FROM_VPN_MARK:x}"),
                "table", &cfg.iproute2_table_index.to_string()]);
    ...
    if has_v4 {
        Command::new("ip")
            .args(["rule", "add", "priority", &prio_base.to_string(),
                "iif", if_name, "goto", &nop_prio.to_string()])
            .output().ok();
        state.rule_priorities.push(prio_base);
    }
    ...
    for addr_str in &cfg.address {
        if let Some((IpAddr::V4(ip), pl)) = parse_addr_prefix(addr_str) {
            ...
            Command::new("ip")
                .args(["rule", "add", "priority", &prio_base.to_string(),
                    "iif", "lo", "from", &format!("{net}/{pl}"), "lookup", "main"])
                .output().ok();
            state.rule_priorities.push(prio_base);
        }
    }
...
pub async fn teardown(cfg: &TunInboundConfig, if_name: &str, state: &SetupState) -> anyhow::Result<()> {
    // 清理路由
