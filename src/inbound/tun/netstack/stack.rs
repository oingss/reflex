...
impl NetStack {
    ...
    pub fn new(
        ...
        let udp_socket = UdpSocket::new(udp_outbound_stack);
        let (tcp_inbound_app, tcp_outbound_stack) = mpsc::unbounded_channel::<Packet>();
        let tcp_listener = TcpListener::new(tcp_outbound_stack, tcp_packet_sender, mtu);
    ...
    pub fn split(self) -> (StackSplitSink, StackSplitStream) {
        (
            ...
            StackSplitStream::new(self.tcp_outbound),
        )
    }
}
...
impl futures::Sink<Packet> for StackSplitSink {
    ...
    fn start_send(mut self: std::pin::Pin<&mut Self>, item: Packet) -> Result<(), Self::Error> {
        ...
        // R3：用 transport() 跳过 IPv6 扩展头后判定协议；旧实现对带扩展头
        // 的包（hop-by-hop / routing / dstopts）会误判协议并静默丢弃。
        let (protocol, _) = match packet.transport() {
            Some(v) => v,
            None => {
                debug!(
                    "tun IP packet ignored (unlocatable transport header, \
...
impl StackSplitStream {
    pub fn new(tcp_outbound: mpsc::Receiver<Packet>) -> Self {
        Self {
            inner: ReceiverStream(tcp_outbound),
        }
    }
}
