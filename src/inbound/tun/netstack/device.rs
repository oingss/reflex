...
impl NetstackDevice {
    /// 创建协议栈设备。
    ///
    /// `mtu` 为 TUN 设备的 MTU，应与上层 `tun::Configuration::mtu` 一致，
    /// 否则大于设备 MTU 的出站包会被 smoltcp 丢弃。
    pub fn new(
        tx_sender: mpsc::Sender<Packet>,
        iface_notifier: mpsc::UnboundedSender<IfaceEvent<'static>>,
        mtu: usize,
...
impl Device for NetstackDevice {
    ...
    fn receive(&mut self, _timestamp: Instant) -> Option<(Self::RxToken<'_>, Self::TxToken<'_>)> {
        ...
        // R9：send 仅在接收端（smoltcp iface task）退出后才失败，此时整个
        // 栈正在关闭；旧实现 expect 会让 GC/读循环 task panic。改为忽略。
