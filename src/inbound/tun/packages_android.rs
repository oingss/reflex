//! Android 包名 → UID 映射。
//!
//! 解析 `/data/system/packages.xml`（含 Android Binary XML），
//! 监听文件变化自动刷新，输出 `package_name → uid` 映射表。
//! 参照 sing-tun `packages_android.go`。
...
impl PackageManager {
    ...
    pub async fn id_by_shared_package(&self, shared_package: &str) -> Option<u32> {
        self.shared_by_package.read().await.get(shared_package).copied()
    }
    ...
    pub async fn resolve_packages(&self, packages: &[String]) -> Vec<u32> {
        ...
        packages.iter().filter_map(|p| map.get(p).copied()).collect()
    }
    ...
    async fn update_packages(&mut self) -> anyhow::Result<()> {
        ...
        parse_packages_xml(&text, &mut id_by_package, &mut shared_by_package,
                          &mut package_by_id, &mut shared_by_id);
    ...
    async fn start_watcher(&mut self) {
        ...
        self.watcher_tx = Some(tx_signal);
        ...
        std::thread::spawn(move || {
            ...
            if watcher.watch(Path::new(&path), notify::RecursiveMode::NonRecursive).is_err() {
                return;
            }
            for event in rx {
        ...
        tokio::spawn(async move {
            while rx_signal.recv().await.is_some() {
                ...
                let text = parse_abx_or_xml(&data).unwrap_or_else(|_| String::from_utf8_lossy(&data).to_string());
                let mut id_map = HashMap::new();
                let mut shared_map = HashMap::new();
                let mut pid_map = HashMap::new();
                let mut sid_map = HashMap::new();
                parse_packages_xml(&text, &mut id_map, &mut shared_map, &mut pid_map, &mut sid_map);
...
fn parse_packages_xml(
    ...
    for line in text.lines() {
        let line = line.trim();
        if let Some(uid) = extract_tag_attr(line, "package", "userId") {
            if let Some(name) = extract_tag_attr(line, "package", "name") {
                id_by_package.insert(name.clone(), uid);
                package_by_id.entry(uid).or_insert_with(|| name.clone());
            }
        }
        if let Some(uid) = extract_tag_attr(line, "shared-user", "userId") {
            if let Some(name) = extract_tag_attr(line, "shared-user", "name") {
...
/// 从 XML 标签中提取属性值。
fn extract_tag_attr(line: &str, tag: &str, attr: &str) -> Option<u32> {
    ...
    let value_str = &line[value_start..value_start + value_end];
...
mod tests {
    ...
    fn test_extract_tag_attr() {
        ...
        assert_eq!(extract_tag_attr(line, "package", "name"), None);
        assert_eq!(extract_tag_attr(line, "package", "userId"), Some(10123));
    }
