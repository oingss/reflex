...
//!
//! 认证方式：SOCKS5 用户名/密码子协商（RFC 1929），可选。
//!
//! 配置写法与 sing-box 一致：
//! ```json
...
async fn handle_socks5(
    ...
    let target = match read_socks5_addr(&mut stream, atyp).await {
        ...
        Err(e) => {
            ...
            let rep = if e.to_string().contains("unknown SOCKS5 atyp") { 0x08 } else { 0x01 };
...
fn parse_socks5_udp(buf: &[u8]) -> anyhow::Result<(Bytes, Target)> {
    ...
    anyhow::ensure!(buf[0] == 0 && buf[1] == 0, "non-zero RSV in SOCKS5 UDP packet");
    let frag = buf[2];
