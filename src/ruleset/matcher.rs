...
impl RuleSet {
    ...
    fn match_domain(&self, domain: &str) -> bool {
        ...
        if self.domain_matcher.is_empty() && self.keywords.is_empty() && self.regexes.is_none() {
            return false;
        }
        ...
        // 1. 精确匹配（lazy 计算 FST key，仅在有 exact FST 时才付出代价）
        if self.domain_matcher.matches_exact_lazy(d) {
            return true;
        }
