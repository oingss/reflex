...
enum DomainMatcher {
    Fst {
        /// `ByteSource` 让 FST 字节既可来自 mmap 零拷贝，也可来自 in-memory 拷贝。
        exact: Option<Set<super::loader::ByteSource>>,
        suffix: Option<Set<super::loader::ByteSource>>,
    },
    Legacy {
...
impl DomainMatcher {
    ...
    /// 无需额外分隔符逻辑。全程不堆分配。
    ///
    /// 当拼接后的 key 超过栈缓冲区长度时（输入域名超长 / label 过多），
    /// 回退到堆分配路径，避免静默截断造成假阴性（漏匹配）。
    fn matches_suffix(&self, domain: &str) -> bool {
        match self {
            DomainMatcher::Fst { suffix, .. } => {
                ...
                // key 总长度 = n（label 字节之和）+ nlabels（每个 label 末尾 '.'）
                const STACK_BUF_LEN: usize = 260;
                let total_key_len = n + nlabels;
                if total_key_len <= STACK_BUF_LEN {
                    // 栈路径：零分配
                    ...
                    for k in 0..nlabels {
                        ...
                        buf[buf_len..buf_len + label.len()].copy_from_slice(label);
                        buf_len += label.len();
                        buf[buf_len] = b'.';
                        buf_len += 1;
                    ...
                    // 堆分配回退路径：超长域名（理论上 RFC 域名不会触发，
                    // 但输入可能不规范；保证正确性优先，不静默截断）
                    let mut buf: Vec<u8> = Vec::with_capacity(total_key_len);
                    for k in 0..nlabels {
                        let label_start = if k + 1 < nlabels {
                            label_ends[k + 1] + 1
                        } else {
...
pub struct RuleSet {
    ...
    /// 关键词原始字符串。
    /// 仅当关键词 ≤ 1（未构建 AC 自动机）时保留，用于线性扫描匹配；
    /// 关键词 ≥ 2 时会构建 [`keyword_automaton`] 并清空本字段以释放内存
    /// （AC 自动机内部已持有 pattern 副本，原 Vec 不再被读取）。
    keywords: Vec<Box<str>>,
    /// 关键词总数（不受 `keywords` 清空影响），用于 `rule_count` 统计与
    /// `has_domain_matchers` 判定。
    keyword_count: usize,
    /// Aho-Corasick 自动机：关键词 ≥ 2 时构建，单次扫描域名即可匹配所有关键词。
    /// 关键词 ≤ 1 时保持线性扫描（AC 有构建开销，单关键词线性更快）。
    ...
    // ── 例外（@@）匹配器 ──────────────────────────────────────────────
    exclude_domain_matcher: DomainMatcher,
...
impl RuleSet {
    pub fn from_loaded(loaded: LoadedRuleSet) -> Result<Self> {
        ...
        // ── 关键词 ──────────────────────────────────────────────────────────
        let keyword_count = loaded.domain_keywords.len();
        let mut keywords: Vec<Box<str>> = loaded
            .domain_keywords
            .into_iter()
            .map(|s| s.into_boxed_str())
            .collect();

        // 关键词 ≥ 2 时构建 Aho-Corasick 自动机，将 O(N×M) 线性扫描优化为单次扫描。
        // AC 构建成功后其内部已持有 pattern 副本，原 keywords Vec 不再被读取，
        // 清空以释放内存（keyword_count 单独保留用于统计/判定）。
        let keyword_automaton = if keywords.len() >= 2 {
            match AhoCorasick::new(keywords.iter().map(|s| s.as_ref())) {
                Ok(ac) => {
                    keywords.clear();
        ...
        let has_domain_matchers =
            !domain_matcher.is_empty() || keyword_count > 0 || regexes.is_some();
        let has_ip_matchers = !ipv4_ranges.is_empty() || !ipv6_ranges.is_empty();
        let has_port_matchers = port_bitset.is_some();

        // ── 例外匹配器 ──────────────────────────────────────────────────────
        let exclude_domain_matcher = if !loaded.exclude_domain_fst.is_empty()
            || !loaded.exclude_domain_suffix_fst.is_empty()
        {
            ...
            // Legacy path for v1 exclude data (unlikely for adguard but kept for completeness)
            let domains = ahash::AHashSet::default();
            let suffix_trie = super::trie::SuffixTrie::new();
            DomainMatcher::Legacy {
                domains,
                suffix_trie,
            }
        ...
        let exclude_regexes = if loaded.exclude_domain_regexes.is_empty() {
            None
        } else {
            Some(
                RegexSet::new(&loaded.exclude_domain_regexes)
                    .map_err(|e| RuleSetError::LoadedInvalidRegex(e.to_string()))?,
            )
        };
    ...
    pub fn rule_count(&self) -> usize {
        ...
        let keyword_count = self.keyword_count;
    ...
    /// 检查域名是否命中例外（@@）规则。命中例外时整体规则集不生效。
    #[inline]
    fn domain_is_excluded(&self, domain: &str) -> bool {
        // exact
        if self.exclude_domain_matcher.matches_exact_lazy(domain) {
            return true;
        }
        // suffix
        if self.exclude_domain_matcher.matches_suffix(domain) {
            return true;
        }
        // regex
        if let Some(ref rs) = self.exclude_regexes {
            if rs.is_match(domain) {
                return true;
            }
        }
...
impl RuleSet {
    pub fn from_text(src: &str) -> Result<Self> {
        ...
        // 直接从 CompiledRuleSet 构建，省 serialize→from_bytes 往返
        let loaded = LoadedRuleSet::from_compiled(compiled)?;
...
mod tests {
    ...
    /// 回归测试：超长后缀的 FST key 超过 260 字节栈缓冲区时，`matches_suffix`
    /// 不得静默截断造成假阴性。
    ///
    /// 旧实现用固定 260B 栈缓冲区拼接查询 key，超长时截断 label，导致后缀规则
    /// （FST key > 260B）对自身 / 子域漏匹配。新实现检测到超长后回退堆分配路径。
    ///
    /// 注：RFC 域名 ≤ 253 字节，编译器（`from_text`）会拒绝超长规则；但
    /// `matches_suffix` 接受任意 `&str` 输入且不做长度校验，故仍需保证超长
    /// 输入的正确性。这里直接用 `build_suffix_fst` 构造一个 259 字节的后缀
    /// （FST key = "com." + "a"*255 + "." = 261 字节，越过 260 栈缓冲区）。
    #[test]
    fn suffix_oversized_domain_no_false_negative() {
        use crate::ruleset::compiler::build_suffix_fst;
        use crate::ruleset::ByteSource;

        // 后缀 = "a"*255 + ".com" = 259 字节（> RFC 253，编译器会拒，故绕过）
        let long_label: String = "a".repeat(255);
        let suffix = format!("{}.com", long_label);
        // FST key = "com." + "a"*255 + "." = 261 字节，触发堆路径
        let fst_bytes = build_suffix_fst(std::slice::from_ref(&suffix)).unwrap();
        assert!(
            fst_bytes.len() > 260,
            "测试前提：FST key 应超过 260 字节栈缓冲区"
        );

        let loaded = crate::ruleset::LoadedRuleSet {
            domain_suffix_fst: ByteSource::Owned(fst_bytes.into()),
            ..Default::default()
        };
        let r = RuleSet::from_loaded(loaded).unwrap();

        // 自身匹配（key 261B，必须走堆路径才能查到）
        assert!(
            r.matches(&MatchTarget::Domain(&suffix)),
            "超长后缀应能匹配自身（堆路径）"
        );
        // 子域匹配
        let sub = format!("sub.{suffix}");
        assert!(
            r.matches(&MatchTarget::Domain(&sub)),
            "超长后缀应能匹配其子域（堆路径）"
        );
        // 无关域名不匹配（label 多一个 a）
        let other = format!("a{suffix}");
        assert!(
            !r.matches(&MatchTarget::Domain(&other)),
            "不同后缀不应误匹配"
        );
    }
}
