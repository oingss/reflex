...
inbound::{
    ...
    http::HttpInbound,
    http_server::HttpServerInbound,
    mixed::MixedInbound,
    naive::NaiveInbound,
    shadowquic::ShadowquicInbound,
    shadowsocks::ShadowsocksInbound,
    socks::SocksInbound,
    socks_server::SocksServerInbound,
    ...
    pub async fn start_with_config_path(
        ...
        for ib_config in inbounds_iter {
            match ib_config {
                ...
                InboundConfig::HttpServer(c) => {
                    info!(tag=%c.tag, listen=%c.listen, port=%c.listen_port, tls=%c.tls.enabled, "starting http-server inbound");
                    let inbound = HttpServerInbound::new(c.clone(), tcp_tx.clone());
                    tasks.spawn(async move { inbound.run().await });
                }
                InboundConfig::SocksServer(c) => {
                    info!(tag=%c.tag, listen=%c.listen, port=%c.listen_port, "starting socks-server inbound");
                    let inbound = SocksServerInbound::new(c.clone(), tcp_tx.clone());
                    tasks.spawn(async move { inbound.run().await });
                }
