...
pub struct ClashApi {
    ...
    /// 用 RwLock 包裹：PATCH /configs 热改 log-level 时同步更新此字段，
    /// 使 GET /configs 能准确反映最新值（旧实现只更新 tracing 原子句柄，
    /// 导致 UI 切级别后 /configs 仍返回初始值）。
    ...
    /// 被禁用规则的下标集合（PATCH /rules/disable 维护）。
    /// 对齐 mihomo `RuleExtra.Disabled`：dashboard 通过 `PATCH /rules/disable`
    /// 传 `{index: bool}` 来切换规则禁用状态，`GET /rules` 的 `extra.disabled`
    /// 字段反映此状态。reflex 的路由规则是配置驱动的不可变结构，禁用状态
    /// 仅在此集合中记录；Router 在匹配前查询此集合跳过被禁用规则。
    disabled_rules: RwLock<HashSet<usize>>,
...
impl ClashApi {
    ...
    pub fn new(
        ...
        Self {
            ...
            log_tx,
            inbound_configs,
            log_level: RwLock::new(log_level),
            rs_registry,
            dns_resolver,
    ...
    async fn handle_request(self: Arc<Self>, request: HttpRequest, mut stream: TcpStream) {
        ...
        let response = match (request.method.as_str(), path) {
            ...
            ("GET", "/logs") => {
                self.get_logs_stream(&mut stream, query).await;
                return;
            }
    ...
    fn get_configs(&self) -> HttpResponse {
        ...
        for ib in &self.inbound_configs {
            let listen = match ib {
                ...
                // 以下协议无 Clash 标准端口字段，仅报告 bind 地址
                IB::Vless(c) => &c.listen,
                IB::Vmess(c) => &c.listen,
                IB::Trojan(c) => &c.listen,
                IB::Shadowsocks(c) => &c.listen,
                IB::Naive(c) => &c.listen,
                IB::Anytls(c) => &c.listen,
                IB::Hysteria2(c) => &c.listen,
                IB::Tuic(c) => &c.listen,
        ...
        let log_level = *self.log_level.read().unwrap_or_else(|e| e.into_inner());
    ...
    fn patch_configs(&self, body: &[u8]) -> HttpResponse {
        ...
        if let Some(mode) = value.get("mode").and_then(|v| v.as_str()) {
            ...
            if valid {
                ...
                // 对齐 sing-box SetMode：模式切换（rule/global/direct）时清空
                // DNS 内存缓存。规则管线会依据模式选不同 DNS 服务器/动作，
                // 旧实现不清缓存导致切换后短暂时间内命中旧模式的缓存结果。
        ...
        if let Some(level) = value.get("log-level").and_then(|v| v.as_str()) {
            // 合法值：trace/debug/info/warning/warn/error/silent
            // （对齐 Clash /configs 响应）。解析后同时更新：
            // 1) tracing 原子句柄（subscriber 侧实时过滤日志）
            // 2) self.log_level（GET /configs 的返回值，旧实现漏了这一步）
            let (u8_val, new_level) = match level {
                "error" => (Some(1), Some(LogLevel::Error)),
                "warning" | "warn" => (Some(2), Some(LogLevel::Warn)),
                "info" => (Some(3), Some(LogLevel::Info)),
                "debug" => (Some(4), Some(LogLevel::Debug)),
                "trace" => (Some(5), Some(LogLevel::Trace)),
                ...
                "silent" => (Some(0), Some(LogLevel::Off)),
                _ => (None, None),
            };
            if let (Some(v), Some(lv)) = (u8_val, new_level) {
                if let Some(handle) = &self.log_level_handle {
                    handle.store(v, Ordering::Relaxed);
                }
                if let Ok(mut cur) = self.log_level.write() {
                    *cur = lv;
                }
            }
        }
    ...
    async fn get_traffic_stream(self: Arc<Self>, stream: &mut TcpStream) {
        ...
        loop {
            ...
            // 对齐 mihomo Traffic{Up,Down,UpTotal,DownTotal}：同时返回每秒增量
            // 与累计总量。zashboard/metacubexd 的流量卡片同时显示实时速率（up/down）
            // 和累计字节（upTotal/downTotal）；旧实现漏掉累计字段导致累计计数器恒为 0。
            let line = serde_json::to_vec(&json!({
                "up": up_delta,
                "down": down_delta,
                "upTotal": snap.bytes_up,
                "downTotal": snap.bytes_down,
            }))
            .unwrap_or_default();
            // chunk 格式：`<hexlen>\r\n<data>\r\n`
    ...
    async fn ws_traffic(self: Arc<Self>, request: HttpRequest, mut stream: TcpStream) {
        ...
        loop {
            ...
            prev_up = snap.bytes_up;
            prev_down = snap.bytes_down;
            // 对齐 mihomo Traffic{Up,Down,UpTotal,DownTotal}（见 get_traffic_stream 注释）
    ...
    async fn get_logs_stream(self: Arc<Self>, stream: &mut TcpStream, query: &str) {
        // 对齐 mihomo getLogs（server.go:482-498）：HTTP 和 WS 路径都解析
        // `?level=` 查询参数作为最低推送级别。旧实现仅 ws_logs 解析 level，
        // HTTP chunked 路径完全忽略，导致用 EventSource / fetch 消费
        // `/logs?level=debug` 的 dashboard 收到所有级别日志，无视过滤选择。
        // 合法值：debug/info/warning/warn/error/silent，默认 info。
        let min_level = query
            .split('&')
            .find_map(|kv| kv.strip_prefix("level="))
            .unwrap_or("info");
        let min_rank = log_level_rank(min_level);
    ...
    async fn ws_connections(self: Arc<Self>, request: HttpRequest, mut stream: TcpStream) {
        ...
        // 对齐 sing-box getConnections：先立即发送一帧快照，再按 interval 定时推送。
        // 旧实现先 sleep 再发帧，导致 dashboard 连接页在首个 interval 内为空。
        if self.send_connections_snapshot(&mut stream).await.is_err() {
            return;
        }
    ...
    /// 发送一帧 /connections 快照（WS 推送用）。
    /// 独立 async fn 而非闭包：闭包无法返回借用局部变量（`msg`）的 future。
    async fn send_connections_snapshot(&self, stream: &mut TcpStream) -> anyhow::Result<()> {
        let snap = self.stats.global_snapshot();
        let conns = self.conn_tracker.snapshot();
        let conn_json: Vec<serde_json::Value> = conns.iter().map(conn_to_json).collect();
        let memory = read_process_rss_kb().unwrap_or(0) * 1024;
    ...
    async fn ws_memory(self: Arc<Self>, request: HttpRequest, mut stream: TcpStream) {
        ...
        loop {
            tokio::time::sleep(Duration::from_secs(1)).await;
            // 首个周期也直接读真实 RSS，避免 UI 首帧闪现 0 再跳变
            let inuse = read_process_rss_kb().unwrap_or(0) * 1024;
            let msg =
                serde_json::to_vec(&json!({"inuse": inuse, "oslimit": 0})).unwrap_or_default();
            if ws_send_text(&mut stream, &msg).await.is_err() {
    ...
    async fn get_dns_query(&self, query: &str) -> HttpResponse {
        ...
        match resolver.handle(msg, "clash-api").await {
            Ok(raw_resp) => {
                let resp = &raw_resp[..];
                // 从响应报文解析 DNS header flags（对齐 sing-box clashapi/dns.go：
                // 直接透传 miekg/dns 的 Rcode/Truncated/RecursionDesired/
                // RecursionAvailable/AuthenticatedData/CheckingDisabled，而不是
                // 硬编码 RD/RA 常量）。flags 位于 header 第 3、4 字节。
                let status = if resp.len() >= 4 { resp[3] & 0x0F } else { 0 };
                let tc = resp.len() >= 4 && (resp[2] & 0x02) != 0;
                let rd = resp.len() >= 4 && (resp[2] & 0x01) != 0;
                let ra = resp.len() >= 4 && (resp[2] & 0x80) != 0;
                let ad = resp.len() >= 4 && (resp[3] & 0x20) != 0;
                let cd = resp.len() >= 4 && (resp[3] & 0x10) != 0;
                let answers = parse_dns_answers(&raw_resp, name);
                let (authority, additional) = parse_dns_sections(&raw_resp, name);
                ...
                // Server 固定 "internal"（对齐 sing-box clashapi/dns.go）。
                ...
                // Authority / Additional 仅在非空时输出（对齐 sing-box：len > 0 才放入）
                if !authority.is_empty() {
                    resp_json["Authority"] = serde_json::Value::Array(authority);
                }
                if !additional.is_empty() {
                    resp_json["Additional"] = serde_json::Value::Array(additional);
                }
                json_response(resp_json)
            }
            // block / predefined 规则的 drop 方法：静默丢弃查询，不返回任何响应。
            // 对齐 sing-box `RuleActionRejectMethodDrop` → `tun.ErrDrop`。
            // Clash API 端无法静默丢弃（必须返回 HTTP 响应），返回 451 以区分。
            Err(e) if e.downcast_ref::<crate::dns::DnsDropError>().is_some() => {
                json_response_status(
                    451,
                    json!({
                        "message": "dns query dropped (block/predefined method=drop)"
                    }),
                )
            }
    ...
    /// PATCH /rules/disable — 切换规则禁用状态（zashboard toggleRuleDisabledAPI）。
    /// 对齐 mihomo `disableRules`（rules.go:76-99）：请求体为 `{index: disabled}`
    /// 的 map，`disabled` 为 true 表示禁用该规则，false 表示启用。
    /// mihomo 仅对实现 `RuleWrapper` 的规则生效；reflex 的路由规则是配置驱动的
    /// 不可变结构，此处把禁用状态记录到 `disabled_rules` 集合，`GET /rules` 的
    /// `extra.disabled` 字段反映此状态。越界 index 静默忽略（与 mihomo 一致）。
    fn toggle_rule_disabled(&self, body: &[u8]) -> HttpResponse {
        // 解析 `{index: bool}` map；空体或非法 JSON 返回 400。
        let payload: std::collections::HashMap<usize, bool> = match serde_json::from_slice(body) {
            Ok(m) => m,
            Err(e) => {
                return text_response(
                    400,
                    "Bad Request",
                    &format!("invalid json (expected {{index: bool}}): {e}"),
                )
            }
        };
        let rule_count = self.route_config.rules.len();
        if let Ok(mut disabled) = self.disabled_rules.write() {
            for (index, disable) in payload {
                // 越界 index 静默忽略（对齐 mihomo：`index >= len(rules)` 时 continue）
                if index >= rule_count {
                    continue;
                }
                if disable {
    ...
    fn get_rules(&self) -> HttpResponse {
        ...
        let mut rules: Vec<serde_json::Value> = self
            ...
            .map(|(index, r)| {
                ...
                } else if r.is_sniff_rule() {
                    ("SNIFF", String::new())
                } else if r.is_hijack_dns() {
                    ("HIJACK-DNS", String::new())
                } else if r.is_resolve_rule() {
                    ("RESOLVE", String::new())
                } else if matches!(
                    r.action,
                    Some(crate::config::route::RouteActionConfig::Reject { .. })
                ) {
                    ("REJECT", String::new())
                } else if matches!(
                    r.action,
                    Some(crate::config::route::RouteActionConfig::Block)
                ) {
                    ("BLOCK", String::new())
                } else {
                    ("MATCH", String::new())
                };
                let proxy = if r.is_hijack_dns() {
                    "dns-out".to_string()
                } else {
                    r.outbound_tag().to_string()
                };
                ...
                let reflex_extra = serde_json::to_value(r).unwrap_or_else(|_| json!({}));
                // 对齐 mihomo Rule{Index,Extra{Disabled,...}}：
                // - `index`：规则在列表中的序号，dashboard 据此构造
                //   `PATCH /rules/disable` 的 `{index: bool}` 请求体；旧实现漏掉
                //   该字段导致 dashboard 无法禁用规则（连请求都发不出）。
                // - `extra.disabled`：是否被 PATCH /rules/disable 禁用；
                //   未禁用时为 false，dashboard 据此渲染开关状态。
                let disabled = self
                    .disabled_rules
                    .read()
        ...
        // 末尾 MATCH 兜底规则：index 为规则列表长度（紧接最后一条显式规则之后）。
        // 对齐 mihomo：MATCH 规则同样有 index，且 extra.disabled=false（不可禁用）。
        let final_index = self.route_config.rules.len();
        rules.push(json!({
            "index": final_index,
            "type": "MATCH",
            "payload": "",
    ...
    async fn serve_ui(&self, path: &str) -> HttpResponse {
        ...
        // 对齐 mihomo（server.go:149）：`GET /ui` 302 重定向到 `/ui/`。
        // 不重定向时浏览器在 `/ui` 加载 index.html，其中的相对资源引用
        // （如 `<script src="assets/app.js">`）会解析到 `/assets/app.js`
        // 而非 `/ui/assets/app.js`，导致 404、dashboard 静态资源加载失败。
        // 仅当 external_ui 已配置时重定向；未配置时走下方 404。
        if relative.is_empty() {
            return HttpResponse::new(302, "Found")
                .header("Location", "/ui/")
                .body(Vec::new(), "text/plain; charset=utf-8");
        }
...
fn conn_to_json(c: &ConnMeta) -> serde_json::Value {
    ...
    //
    // source_ip / destination_ip 可能是 IPv4-mapped IPv6 形式（双栈 socket
    // 上 IPv4 连接的内核映射，形如 "::ffff:10.0.0.101"），UI 上显示不友好，
    // 这里统一转回纯 IPv4。
    let source_ip = clean_ip_str(&c.source_ip);
    let destination_ip = clean_ip_str(&c.destination_ip);
    json!({
        "id": c.id.to_string(),
        "metadata": {
            "network": c.network,
        ...
        // 对齐 mihomo TrackerInfo.ProviderChain (json:"providerChains")：
        // reflex 不支持 provider chain，返回空数组。metacubexd 等前端会
        // `conn.providerChains?.length` 或迭代，缺失该字段时部分版本会报错。
        "providerChains": [],
        "rule": &*c.rule,
...
/// 将 IP 字符串规范化为纯 IPv4 显示：
/// 双栈 socket 上 IPv4 连接的内核地址是 IPv4-mapped IPv6（如 "::ffff:10.0.0.101"），
/// 这里若解析为 IPv6 且是 mapped 形式则转回 IPv4；其余原样返回。
...
/// 日志级别排序：数值越大级别越高，用于 /logs 的 `?level=` 过滤。
/// 对齐 mihomo `log.LogLevel`：debug < info < warning < error，silent 全屏蔽。
/// 被 `get_logs_stream`（HTTP chunked）和 `ws_logs`（WebSocket）共用，
/// 保证两条路径过滤行为一致。
fn log_level_rank(level: &str) -> usize {
    match level {
        ...
        "warning" | "warn" => 2,
        "error" => 3,
        // silent 或未知：返回 usize::MAX，min_rank 永远 >= 任何实际级别 → 全屏蔽
        _ => usize::MAX,
    }
}
...
/// Linux 读 /proc/self/status；Windows 用 GetProcessMemoryInfo；
/// 其他平台返回 None（由调用方回退为 0）。
pub(crate) fn read_process_rss_kb() -> Option<u64> {
    ...
    {
        // GetProcessMemoryInfo(GetCurrentProcess()) 返回当前进程的工作集大小
        // （WorkingSetSize，字节），/1024 得到 kB。GetCurrentProcess 返回的
        // 伪句柄不需要也不应该 CloseHandle。
        ...
        unsafe {
            ...
            if GetProcessMemoryInfo(process, &mut pmc, cb).is_ok() {
                return Some((pmc.WorkingSetSize / 1024) as u64);
            }
        }
        None
    }
...
fn parse_dns_answers(raw: &[u8], query_name: &str) -> Vec<serde_json::Value> {
    ...
    let (records, _next) = parse_dns_records(raw, pos, ancount, query_name);
    records
}

/// 解析 Authority（nscount）与 Additional（arcount）两个 section，
/// 供 /dns/query 对齐 sing-box 输出（非空时才放进响应）。
/// 返回 (authority_records, additional_records)。
fn parse_dns_sections(
    raw: &[u8],
    query_name: &str,
    ...
    let (_, pos) = parse_dns_records(raw, pos, ancount, query_name);
    let (authority, pos) = parse_dns_records(raw, pos, nscount, query_name);
    let (additional, _pos) = parse_dns_records(raw, pos, arcount, query_name);
    (authority, additional)
}

/// 解析一段 DNS RR 记录（Answer/Authority/Additional 通用），返回记录列表与
/// 解析结束位置。记录格式：[NAME][TYPE(2)][CLASS(2)][TTL(4)][RDLENGTH(2)][RDATA]。
...
mod tests {
    ...
    /// /logs 的 `?level=` 过滤排序逻辑（HTTP chunked 与 WS 共用）。
    /// 对齐 mihomo `log.LogLevel`：debug < info < warning < error，silent 全屏蔽。
    #[test]
    fn log_level_rank_ordering() {
        // 数值越大级别越高
        assert!(log_level_rank("debug") < log_level_rank("info"));
        assert!(log_level_rank("info") < log_level_rank("warning"));
        assert!(log_level_rank("warning") == log_level_rank("warn"));
        assert!(log_level_rank("warn") < log_level_rank("error"));

        // silent / 未知 → usize::MAX（全屏蔽）
        assert_eq!(log_level_rank("silent"), usize::MAX);
        assert_eq!(log_level_rank("bogus"), usize::MAX);
        ...
        assert!(
            log_level_rank("error") >= min_rank,
            "error 应通过 info 级别过滤"
        );

        // min_rank=debug 时所有实际级别都通过（debug 是最低实际级别）
        let debug_min = log_level_rank("debug");
        assert!(log_level_rank("debug") >= debug_min);
        assert!(log_level_rank("error") >= debug_min);

        // min_rank=silent 时全部被过滤（包括 error）
        let silent_min = log_level_rank("silent");
        assert!(log_level_rank("error") < silent_min, "silent 应屏蔽 error");
    }
}
