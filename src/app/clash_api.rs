...
// 字节计数器必须 64 位，否则累计流量超 4 GB（i32）/ 2 GB（i64 的一半）即回绕。
// 部分 target（如 mips/mipsel-unknown-linux-musl）没有原生 64 位原子操作，
// std::sync::atomic::AtomicI64/AtomicU64 在这些平台上不存在。portable-atomic
// 在硬件支持时映射到原生指令，不支持时透明退化为锁实现，接口与 std 一致，
// 因此这里统一使用 64 位计数器，不再需要按平台切换宽度。
