...
/// 服务器先开口的协议端口集合（对齐 sing-box common/sniff/sniff.go:25-39 的
/// `Skip`）：SMTP(25,465,587) / IMAP(143,993) / POP3(110,995)。
///
/// 这些协议由服务端先发送数据，客户端侧没有首包可读；嗅探会一直阻塞到超时
/// （默认 300ms），白白增加每条连接的延迟。命中时调用方应直接跳过嗅探。
pub fn is_server_first_port(port: u16) -> bool {
...
// ── SniffOutcome：单协议嗅探内部结果（对齐 sing-box ErrNeedMoreData 模式）──────
//
// sing-box 的 PeekStream 用 ErrNeedMoreData 信号驱动循环读取：sniffer 返回
// ErrNeedMoreData → 主循环再读一批数据 → 重试；sniffer 返回其他 error →
// 不再重试该 sniffer。reflex 旧实现用 Option<SniffResult>，None 既表示
// "不匹配"也表示"数据不足"，主循环只能每次重试**全部** sniffer。
//
// SniffOutcome 把两种语义分开：
// - Matched：命中协议，返回结果
// - NeedMoreData：协议前缀匹配但数据不足，需要继续读取
// - NotMatched：明确不匹配，不再尝试
//
// 这样主循环可以：
// 1. 首字节过滤：根据第一个字节跳过明显不匹配的协议（如 TLS 首字节必须是 0x16）
// 2. 候选裁剪：返回 NotMatched 的协议从候选列表移除，不再重试
// 3. 提前终止：所有候选都返回 NotMatched 时立即终止，不再无谓读取
...
/// TCP 流首字节快速过滤：根据第一个字节判断协议是否可能匹配。
///
/// 对齐 sing-box PeekStream 中 sniffer 各自做前缀检查的思路，
/// 但把检查提取到主循环，避免对明显不匹配的协议做完整解析。
fn first_byte_matches_tcp(first: u8, t: SniffType) -> bool {
    match t {
        SniffType::Tls => first == 0x16,
        SniffType::Http => first.is_ascii_uppercase(),
        SniffType::Quic => first & 0xC0 == 0xC0,
        SniffType::Ssh => first == b'S',
        SniffType::BitTorrent => first == 0x13,
        SniffType::Dns => true, // DNS over TCP: 2-byte length prefix, any byte
        SniffType::Rdp => first == 0x03,
        // UDP-only protocols — keep true for safety (parser will reject)
        SniffType::Dtls => true,
        SniffType::Stun => true,
        SniffType::Ntp => true,
    }
...
/// 在 `haystack` 中查找 `needle`，返回首次出现位置
...
/// 对 `stream` 进行非破坏性协议嗅探。
///
/// - `sniff_types`：为空时使用默认协议列表（TLS/HTTP/QUIC）
/// - 读出最多 [`PEEK_BUF_SIZE`] 字节，解析后通过 `stream.prepend()` 归还
///
/// ## 优化（对齐 sing-box PeekStream + ErrNeedMoreData 模式）
///
/// 1. **首字节过滤**：首次读到数据后，根据第一个字节用 [`first_byte_matches_tcp`]
///    过滤候选协议列表。例如 TLS 首字节必须是 0x16，HTTP 首字节必须是大写 ASCII
///    字母。这样对一条 TLS 连接，HTTP/SSH/BT/RDP 等 sniffer 在首轮就被跳过。
/// 2. **NeedMoreData 驱动**：每个 sniffer 返回 [`SniffOutcome`] 而非 `Option`，
///    区分"数据不足"和"明确不匹配"。返回 `NotMatched` 的协议从候选列表移除，
///    不再重试。所有候选都 `NotMatched` 时立即终止。
/// 3. **循环读取 + deadline**：用 `tokio::time::Instant` 设定总 deadline，
///    每次用 `timeout_at(deadline, read)` 读取，避免阻塞超过总超时。
pub async fn sniff(
    ...
    // 候选列表：首次读到数据后用首字节过滤；之后仅保留返回 NeedMoreData 的协议
    let mut candidates: Vec<SniffType> = effective_types.to_vec();
    ...
    loop {
        ...
        // 首次读到数据后，用首字节快速过滤候选
        if total == n {
            candidates.retain(|t| first_byte_matches_tcp(buf[0], *t));
        }

        // 尝试所有候选协议
        let mut next_candidates = Vec::new();
        for &sniff_type in &candidates {
            let outcome = match sniff_type {
            ...
            match outcome {
                ...
                SniffOutcome::NeedMoreData => {
                    next_candidates.push(sniff_type);
                }
...
/// 返回 `(protocol, domain)`。
///
/// UDP 是单个完整数据报，不存在"数据不足"的情况，因此 NeedMoreData
/// 等同于 NotMatched。
pub fn sniff_packet(data: &[u8], sniff_types: &[SniffType]) -> Option<SniffResult> {
    ...
    for sniff_type in effective_types {
        let outcome = match sniff_type {
            ...
            SniffType::Tls | SniffType::Http | SniffType::Ssh | SniffType::Rdp => {
                SniffOutcome::NotMatched
            }
        };
        if let SniffOutcome::Matched(r) = outcome {
            return Some(r);
        }
    }
    None
...
fn try_tls(buf: &[u8]) -> SniffOutcome {
    // 逐步检查：先验证前缀字节，再检查数据量
    if buf.is_empty() {
    ...
    if buf[5] != 0x01 {
        return SniffOutcome::NotMatched; // TLS but not ClientHello
    }
    if buf.len() < 43 {
        return SniffOutcome::NeedMoreData;
    }
    ...
    if pos + 2 > buf.len() {
        return SniffOutcome::Matched(SniffResult {
            domain: None,
            protocol: "tls",
        });
    }
    ...
    SniffOutcome::Matched(SniffResult {
        domain: sni,
...
// ── HTTP/1.x Host 解析 ───────────────────────────────────────────────────────
//
// 优化：用字节级扫描替代 std::str::from_utf8 + split("\r\n") + to_ascii_lowercase，
// 避免在热路径上产生堆分配。对齐 sing-box HTTPHost 的语义：
// - 未找到 \r\n → NeedMoreData（可能是 HTTP，首行不完整）
// - 首行不含 " HTTP/" → NotMatched
// - 找到 \r\n\r\n（headers 结束）但无 Host → NotMatched
// - headers 不完整 → NeedMoreData
// - 找到 Host → Matched

fn try_http_host(buf: &[u8]) -> SniffOutcome {
    if buf.is_empty() {
        return SniffOutcome::NeedMoreData;
    }
    // 快速排除：HTTP 方法首字节必须是大写 ASCII 字母
    if !buf[0].is_ascii_uppercase() {
        return SniffOutcome::NotMatched;
    }

    // 查找第一个 \r\n（首行结束）
    let crlf_pos = match find_subseq(buf, b"\r\n") {
        Some(pos) => pos,
        None => return SniffOutcome::NeedMoreData, // 首行不完整
    };

    // 首行必须包含 " HTTP/"
    if find_subseq(&buf[..crlf_pos], b" HTTP/").is_none() {
        return SniffOutcome::NotMatched;
    }

    // 逐行扫描 headers，查找 Host:
    let mut pos = crlf_pos + 2;
    while pos < buf.len() {
        // 查找当前行的结束 \r\n
        let line_end = match find_subseq(&buf[pos..], b"\r\n") {
            Some(rel) => pos + rel,
            None => return SniffOutcome::NeedMoreData, // headers 不完整
        };

        if line_end == pos {
            // 空行 = headers 结束，未找到 Host
            return SniffOutcome::NotMatched;
        }
        ...
        if line.len() >= 5 && line[..5].eq_ignore_ascii_case(b"Host:") {
            let host = std::str::from_utf8(&line[5..]).unwrap_or("").trim();
...
fn try_quic(buf: &[u8]) -> SniffOutcome {
    ...
    let (is_v2, initial_salt) = match version {
        ...
        _ => return SniffOutcome::NotMatched,
    };
    ...
    // Token
    let (token_len, vl) = match read_varint(buf, pos) {
        Some(v) => v,
        None => return SniffOutcome::NeedMoreData,
    };
    pos += vl + token_len as usize;

    // Packet Length
    let (pkt_len, vl2) = match read_varint(buf, pos) {
        Some(v) => v,
        None => return SniffOutcome::NeedMoreData,
    };
    pos += vl2;
    ...
    let plaintext = match decrypt_quic_initial(
        dcid,
        initial_salt,
        buf,
        pn_offset,
        ...
        Some(p) => p,
        None => return SniffOutcome::NotMatched,
    };

    // 从解密后的 QUIC CRYPTO frame 中提取 TLS ClientHello
    match extract_sni_from_quic_crypto(&plaintext) {
        Some(r) => SniffOutcome::Matched(r),
...
fn extract_sni_from_quic_crypto(data: &[u8]) -> Option<SniffResult> {
    ...
    if reassembled.len() >= 4 && reassembled[0] == 0x01 {
        ...
        return match try_tls(&fake_record) {
            SniffOutcome::Matched(r) => Some(r),
            _ => None,
        };
    }
...
fn try_ssh(buf: &[u8]) -> SniffOutcome {
    const SSH_PREFIX: &[u8] = b"SSH-";
    if buf.starts_with(SSH_PREFIX) {
        SniffOutcome::Matched(SniffResult {
            domain: None,
            protocol: "ssh",
        })
    } else if buf.len() < SSH_PREFIX.len() && SSH_PREFIX.starts_with(buf) {
        // 部分匹配：可能是 SSH 的前几个字节
        SniffOutcome::NeedMoreData
    } else {
        SniffOutcome::NotMatched
...
fn try_bittorrent(buf: &[u8]) -> SniffOutcome {
    ...
    } else if buf.len() < BT_HEADER.len() && BT_HEADER.starts_with(buf) {
        SniffOutcome::NeedMoreData
    } else {
        SniffOutcome::NotMatched
    }
...
fn try_bittorrent_udp(buf: &[u8]) -> SniffOutcome {
...
fn try_dns_stream(buf: &[u8]) -> SniffOutcome {
    if buf.len() < 2 {
        return SniffOutcome::NeedMoreData;
    }
...
fn try_dns_packet(buf: &[u8]) -> SniffOutcome {
    if is_dns_wire(buf) {
        SniffOutcome::Matched(SniffResult {
            domain: None,
            protocol: "dns",
...
fn try_dtls(buf: &[u8]) -> SniffOutcome {
...
fn try_stun(buf: &[u8]) -> SniffOutcome {
    if buf.len() < 20 {
        return SniffOutcome::NotMatched;
    }
...
fn try_ntp(buf: &[u8]) -> SniffOutcome {
    if buf.len() < 48 {
        return SniffOutcome::NotMatched;
    }
...
fn try_rdp(buf: &[u8]) -> SniffOutcome {
...
mod tests {
    ...
    fn domain(r: &SniffOutcome) -> Option<String> {
        match r {
            SniffOutcome::Matched(r) => r.domain.clone(),
            _ => None,
        }
    }
    fn protocol(r: &SniffOutcome) -> Option<&'static str> {
        match r {
            SniffOutcome::Matched(r) => Some(r.protocol),
            _ => None,
        }
    }
    fn matched(r: &SniffOutcome) -> bool {
        matches!(r, SniffOutcome::Matched(_))
    }
    ...
    #[test]
    fn not_http_returns_not_matched() {
        let data = b"\x16\x03\x01 not tls either";
        assert!(!matched(&try_http_host(data)));
    }

    #[test]
    fn http_incomplete_returns_need_more_data() {
        // First line not complete
        let data = b"GET / HTTP/1.1";
        assert!(matches!(try_http_host(data), SniffOutcome::NeedMoreData));
    ...
    #[test]
    fn tls_too_short_returns_need_more_data() {
        let data = b"\x16\x03\x01\x00\x05\x01";
        assert!(matches!(try_tls(data), SniffOutcome::NeedMoreData));
    }

    #[test]
    fn tls_wrong_first_byte_returns_not_matched() {
        let data = b"GET / HTTP/1.1\r\nHost: example.com\r\n\r\n";
        assert!(matches!(try_tls(data), SniffOutcome::NotMatched));
    }
    ...
    fn dtls_detection() {
        ...
        assert!(!matched(&try_dtls(&not_dtls)));
    }
    ...
    fn utp_detection() {
        ...
        assert_eq!(protocol(&result), Some("bittorrent"));
    }
    ...
    fn first_byte_dispatch() {
        ...
        // QUIC
        assert!(first_byte_matches_tcp(0xC0, SniffType::Quic));
        assert!(first_byte_matches_tcp(0xFF, SniffType::Quic));
        assert!(!first_byte_matches_tcp(0x16, SniffType::Quic));
        // SSH
        assert!(first_byte_matches_tcp(b'S', SniffType::Ssh));
        assert!(!first_byte_matches_tcp(b'G', SniffType::Ssh));
        // BitTorrent
        assert!(first_byte_matches_tcp(0x13, SniffType::BitTorrent));
        assert!(!first_byte_matches_tcp(0x14, SniffType::BitTorrent));
        // RDP
        assert!(first_byte_matches_tcp(0x03, SniffType::Rdp));
        assert!(!first_byte_matches_tcp(0x04, SniffType::Rdp));
    }
}
