...
mod sniff_filter_tests {
    ...
    fn bad_cidr_dropped() {
        let filter = SniffFilter::from_config(
            vec![],
            vec![],
            vec!["not_an_ip".into(), "192.168.0.0/notanum".into(), "10.0.0.0/8".into()],
        );
...
fn try_quic(buf: &[u8]) -> Option<SniffResult> {
    ...
    let plaintext = decrypt_quic_initial(dcid, initial_salt, buf, pn_offset, encrypted_payload.len(), is_v2)?;
...
mod tests {
    ...
    fn tls_without_sni_returns_tls() {
        ...
        body.push(0x00); body.push(0x02); // Cipher suites length = 2
        body.push(0x00); body.push(0x2f); // TLS_RSA_WITH_AES_128_CBC_SHA
