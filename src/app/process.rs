//! 进程查找：根据连接的 (src_ip, src_port, dst_ip, dst_port, protocol)
//! 反查本地进程名和完整路径。
//!
//! 实现参照 clash-rs 的 `sock2proc` crate（`clash-lib/src/app/router/rules/process.rs`）
//! 与 sing-box 的 `common/process` 模块。
//!
//! ## 平台支持
//!
//! - **Linux**：解析 `/proc/net/tcp` / `/proc/net/tcp6` / `/proc/net/udp` /
//!   `/proc/net/udp6`，匹配 `local_address:local_port` 对应的 inode，
//!   再遍历 `/proc/<pid>/fd/*` 找到引用该 inode 的进程，最后读
//!   `/proc/<pid>/comm` 取进程名、`/proc/<pid>/exe` 读完整路径。
//!
//! - **macOS / Windows / 其他平台**：返回 `None`，规则不命中。
//!   未来可通过 `nix` 或 `windows` crate 扩展实现。
//!
//! ## 缓存
//!
//! 进程查找是 syscall 密集型操作（读 /proc、读多个 fd 链接），
//! 同一个 (src, dst, proto) 五元组在连接存活期间进程不变，
//! 因此用一个带 TTL 的 LRU 缓存避免每次路由匹配都做完整查找。
//!
//! 缓存 key = (src, dst, proto)，value = (ProcessInfo, instant)，
//! TTL 默认 5 秒（进程切换是非常罕见的事件）。
...
impl ProcessResolver {
    pub fn new(ttl: Duration) -> Self {
        Self {
            #[cfg(target_os = "linux")]
            cache: RwLock::new(lru::LruCache::new(std::num::NonZeroUsize::new(1024).unwrap())),
            ttl,
        }
    }
...
fn lookup_proc_linux(local: &SocketAddr, proto: &NetProtocol) -> Option<ProcessInfo> {
    ...
    // 3. 读 /proc/<pid>/comm 取进程名
    let name = std::fs::read_to_string(format!("/proc/{pid}/comm"))
        .ok()?
        .trim_end_matches('\n')
        .to_string();

    // 4. 读 /proc/<pid>/exe 取完整路径（可能因权限不足失败，所以是 Option）
    ...
    match addr {
        ...
        SocketAddr::V6(v6) => {
            ...
            for chunk in octets.chunks_exact(4) {
                ...
                hex.push_str(&format!("{:02X}{:02X}{:02X}{:02X}", chunk[3], chunk[2], chunk[1], chunk[0]));
            }
