...
impl Dispatcher {
    ...
    pub async fn run_tcp(self, mut rx: mpsc::Receiver<InboundTcpStream>) {
        while let Some(mut conn) = rx.recv().await {
            ...
            // ── 单遍路由：循环执行非最终动作（sniff/resolve）直到最终动作 ──
            // 对齐 sing-box：sniff/resolve 命中执行后继续匹配下一条规则（不
            // 回退），最终动作（route/reject/block/hijack-dns）命中即终止。
            // RouteState 记录游标与已执行动作，避免重复遍历与重复嗅探/解析。
            let mut route_state = crate::router::RouteState::default();
            // resolve 解析出的 IP，作为新候选参与后续规则的 IP 匹配
            let mut resolved_ip: Option<std::net::IpAddr> = None;
            // TCP 53 端口 DNS 兜底检测只做一次
            let mut dns_probe_done = false;
            // loop 的每个 break 路径都会赋值，初始值仅为占位
            #[allow(unused_assignments)]
            let mut route_options = RouteOptions::default();
            // 优化：延迟 RuleInfo 构造到路由最终确定后，避免循环里重复分配
            // Arc<str>。用 (&str, &str) 累积最终结果，最后一次性 .into() 构造。
            // router 返回的 &str 借用 self.router（Arc<Router>），在循环里存活。
            #[allow(unused_assignments)]
            let mut rule_display: (&str, &str) = ("", "");
            let action = loop {
                ...
                if !dns_probe_done && conn.target.port() == 53 && conn.sniffed_protocol.is_none()
                {
                    ...
                    if let Some(result) = sniff(&mut conn.stream, 0, &dns_only).await {
                        ...
                        // DNS 协议嗅探不携带域名，result.domain 必为 None
                        let _ = result.domain;
                        debug!(target = %conn.target, "tcp dns probe: detected dns protocol");
                        continue;
                    }
                }

                let result = self.router.route_with_state(
                    &mut route_state,
                    &conn.inbound_tag,
                ...
                match result {
                    ...
                    None => {
                        ...
                        break self.router.default_action().clone();
                    }
                }
            };
        ...
        loop {
            tokio::select! {
                maybe_packet = rx.recv() => {
                    ...
                    // ── 单遍路由：循环执行非最终动作（sniff/resolve）直到最终动作 ──
                    // 与 TCP 路径对称（UDP 的 DNS 协议检测已在上方无条件完成，
                    // 无需 53 兜底检测）。
                    let mut route_state = crate::router::RouteState::default();
                    let mut resolved_ip: Option<std::net::IpAddr> = None;
                    // loop 的每个 break 路径都会赋值，初始值仅为占位
                    ...
                    let action = loop {
                        ...
                        match result {
                            Some((action, rt, rp, ro)) => {
                                ...
                                // UDP 嗅探：对单包做协议识别（QUIC/DTLS/STUN/NTP/
                                // BitTorrent-uTP/DNS）。开销极低（只读已有
                                // packet.data，不阻塞 IO）。
                                let filter = crate::app::sniff::SniffFilter::from_config(
                                    force_domain.clone(),
                                    skip_domain.clone(),
                                ...
                                if should_sniff {
                                        ...
                                        crate::app::sniff::sniff_packet(&packet.data, sniff_types)
                                    {
                                        if packet.sniffed_protocol.is_none() {
                                            packet.sniffed_protocol =
                                                Some(result.protocol.to_string());
                                        }
                                        if let Some(domain) = result.domain {
                                    ...
                                    match resolve_result {
                                        Ok(ip) => {
                                            debug!(
                                                domain = %host,
                                                ip = %ip,
                                                "resolve(udp): domain resolved, continuing routing with resolved IP as candidate"
                                            );
                                            resolved_ip = Some(ip);
                                        }
                                        Err(e) => {
...
async fn dispatch_tcp(
    ...
    match action {
        ...
        RouteAction::Reject { method } => {
            debug!(target=%conn.target, method=?method, "tcp rejected by rule");
            reject_tcp(conn, method);
            Ok(())
        }
        RouteAction::Block => {
            debug!(target=%conn.target, "tcp blocked by rule");
            drop(conn.stream);
            Ok(())
        }
    }
...
/// 按指定方式拒绝 TCP 连接（对齐 sing-box route/rule/rule_action.go:374-403
/// 的 `RuleActionReject.Error()` 与 route/route.go:126-131 的 TCP 拒绝路径）：
/// - `default`：尽力发送 RST（SO_LINGER=0 后关闭），对应 sing-box
///   `RejectedError{tun.ErrReset}`；失败则退化为普通关闭
/// - `drop`：静默关闭（FIN/EOF），不主动发 RST，对应 sing-box
///   `RejectedError{tun.ErrDrop}`
/// - `reply`：同 `default`（发 RST）。sing-box 在 TCP 上拒绝 `reply` 方法
///   （route/route.go:128-130 报错）；reflex 保留为可用的 RST 语义，不报错。
#[allow(deprecated)] // 有意使用 SO_LINGER=0 触发 RST(linger 0 不阻塞线程)
fn reject_tcp(conn: InboundTcpStream, method: RejectMethod) {
    match method {
        RejectMethod::Default | RejectMethod::Reply => {
            // 设置 linger 0 后关闭，内核会发 RST 而非 FIN，客户端立即感知拒绝。
            // 失败（平台不支持等）时忽略，退化为普通关闭。
            // 对齐 sing-box `default` → tun.ErrReset。
            // 内层流已泛化（可能为 TLS 解密流），RST 语义通过 raw_tcp 克隆实现；
            // 非 TCP 底层（当前不存在）退化为普通关闭。
            if let Some(raw) = conn.stream.raw_tcp() {
                #[allow(deprecated)]
...
async fn dispatch_udp(
    ...
    match action {
        ...
        RouteAction::Reject { method } => {
            // UDP 无连接概念：丢弃包，不回任何数据。sing-box `default` 方式会尝试
            // 回 ICMP port-unreachable，但那需要原始套接字权限，这里不实现（与
            // block 出站注释保持一致）。
            debug!(target=%packet.target, method=?method, "udp rejected by rule, dropping packet");
            Ok(())
        }
        RouteAction::Block => {
            debug!(target=%packet.target, "udp blocked by rule, dropping packet");
            Ok(())
        }
    }
}
