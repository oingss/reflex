        ...
        while let Some(mut conn) = rx.recv().await {
            ...
            if self.router.hijack_dns_global() && conn.target.port() == 53 {
                ...
                tokio::spawn(async move {
                    if let Err(e) = dispatch_tcp(
                        conn, action, rule_info, mgr, dns_tx, stats, conn_tracker,
                    )
                    .await
                    {
            ...
            let action = if let RouteAction::Resolve { server } = &action {
                ...
                let domain_to_resolve = conn
                    .sniffed_domain
                    .clone()
                    .or_else(|| match &conn.target {
                ...
                if let Some(host) = domain_to_resolve {
                    ...
                    match resolve_result {
                        Ok(ip) => {
                            ...
                            let (a, rt, rp, ro) = self
                                .router
                                .route_tcp_after_resolve(&conn, Some(ip), proc_info.as_ref());
                            rule_display = (rt, rp);
                            route_options = ro.clone();
                            a.clone()
                        }
                        Err(e) => {
                            ...
                            let (a, rt, rp, ro) =
                                self.router.route_tcp_after_resolve(&conn, None, proc_info.as_ref());
                            rule_display = (rt, rp);
                            route_options = ro.clone();
                            a.clone()
                        }
