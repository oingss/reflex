...
//!
//! ## 热更新（P1-2）
//! `start_watchers()` 启动两类后台任务：
//! - **本地规则集**（`type = local`）：用 `notify` crate 监听文件变更，
//!   文件被修改后自动重新编译并刷新元数据。
//! - **远程规则集**（`type = remote` 且配置了 `update_interval`）：按周期
//!   定时调用 `reload_remote()` 重新下载并刷新缓存。
//!
//! 注意：reload 后路由规则本身不会热更新到正在运行的 Router 实例中
//! （需要重建 Router 才能让 CompiledRule 引用的 Arc<RuleSet> 生效）；
//! 此功能主要用于更新缓存文件与元数据，下次重启时加载最新规则集，
//! 同时 DNS 模块的 fakeip/share 查询可以读到最新的 rulesets。
//!
//! 但对于**本地 source 格式**的规则集，热重载会同步刷新 Registry
//! 持有的内存编译结果，使 Clash API `/rules` 等接口能立即反映变更。
...
impl RuleSetRegistry {
    ...
    pub fn start_watchers(self: &Arc<Self>) -> Vec<tokio::task::JoinHandle<()>> {
        ...
        if !handles.is_empty() {
            info!(count = handles.len(), "ruleset: hot-reload watchers started");
        }
        handles
    }
            ...
            move |res: notify::Result<notify::Event>| {
                if let Ok(ev) = res {
                    ...
                    let affects_target = ev.paths.iter().any(|pp| {
                        pp.file_name().map(|n| n == file_name.as_os_str()).unwrap_or(false)
                    });
                    if affects_target {
                        // 通道满时丢弃事件（去抖动会兜底）
    ...
    pub async fn reload_local(
        ...
        let rule_count = tokio::task::spawn_blocking(move || {
            load_and_count(&path_owned, &format)
        })
        .await
        ...
        let rule_count = if format == RuleSetFormat::Source {
            ...
            let compiled = if trimmed.starts_with('{') {
                ...
                crate::ruleset::compiler::CompiledRuleSet::from_mihomo_yaml(trimmed, None)
                    .map_err(|e| anyhow::anyhow!("rule_set '{tag}': mihomo yaml parse error: {e}"))?
            } else {
