//! DNS 上游实现：UDP、TCP、DoH（HTTP/1.1 + HTTP/2 + TLS）、DoT、DoQ、DoH3、
//! hosts、local、内置 rcode、FakeIP。
//!
...
//!   - DoH3 → 同 DoQ，QUIC 依赖 UDP，detour 存在时忽略并直连
//!   - hosts / local → 本地内置上游，不感知 detour，直接读写本地文件 + 系统 socket
...
// 仅 fakeip / udp 模块导出 pub 类型（FakeIpStore、UdpState），它们作为
// UpstreamKind 字段类型的一部分对外可见，因此需要 pub use 重导出。
// hosts / local 也导出 pub 类型（HostsUpstream、LocalUpstream）。
// 其他子模块只有 pub(super) 项，用私有 use 即可。
use doh::*;
use doq::*;
use dot::*;
pub use fakeip::*;
...
pub enum UpstreamKind {
    ...
    Dot {
        ...
        /// Box 装箱以缩小 enum 变体大小（避免 large_enum_variant）。
        conn_pool:
            Box<tokio::sync::Mutex<Option<tokio_rustls::client::TlsStream<tokio::net::TcpStream>>>>,
    },
...
impl DnsUpstream {
    ...
    pub fn query(
        ...
        Box::pin(async move {
            ...
            // 对齐 sing-box client.go:123-129：查询前注入 EDNS0_SUBNET OPT 选项。
            // Rcode / FakeIp / Hosts / Local 是本地内置上游，不注入（避免无意义的 OPT 包装）。
            let msg = if let Some((subnet, prefix_len)) = &self.client_subnet {
