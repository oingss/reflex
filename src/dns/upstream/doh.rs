//! DNS-over-HTTPS 协议实现：HTTP/2（优先）→ 回退 HTTP/1.1，Content-Type: application/dns-message。
//!
...
///
/// DoH 使用 HTTP request/response 配对，不依赖 DNS ID 区分查询；某些严格的
/// DoH 服务器（如 Cloudflare）会拒绝 ID != 0 的请求。返回 (重写后的报文, 原始 ID)
...
/// 将响应的 ID 还原为原始查询 ID（`set_doh_query_id_zero` 的逆操作）。
/// 多数 DoH 服务器返回 ID=0，但即使非 0 也强制覆盖，确保 caller 看到的
