...
///
/// 供 server 级 `DnsServerConfig.client_subnet` 与 per-rule
/// `DnsRuleAction::Route.client_subnet` 共用，故为 `pub(crate)`。
pub(crate) fn parse_client_subnet(s: &str) -> Option<(std::net::IpAddr, u8)> {
