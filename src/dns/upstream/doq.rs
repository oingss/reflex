//! DNS-over-QUIC 协议实现（RFC 9250）：带连接池的 DoQ 查询，2 字节长度前缀帧。
//!
//! 对齐 sing-box `dns/transport/quic/quic.go`：
//! - **endpoint 必须与 connection 共存亡**（修复旧实现 `drop(endpoint)` 致连接失效 bug）：
//!   quinn 的 `Endpoint` 持有底层 UDP socket，endpoint drop 时 socket 也 drop，
//!   QUIC 连接因无法收发包而失效。pool 内同时保存 endpoint + connection。
//! - **失败重试**（对齐 sing-box `for range 2` + `isQUICRetryError`）：
//!   连接级错误（idle timeout / stateless reset / connection lost）时清空 pool，
//!   重新建立连接再试一次。
//! - **IsAlive 检查**：复用前检查 `conn.close_reason()`，对齐 sing-box `IsAlive` 回调。
//! - **Message ID MUST be 0**（RFC 9250 §4.2 强制要求，对齐 sing-box
//!   `quic.go:159` 调用 `WriteMessage(stream, 0, message)`）：
//!   QUIC stream 提供 1:1 query-response 映射，不依赖 DNS Message ID 配对，
//!   RFC 9250 强制 ID=0。查询前重写为 0，响应恢复为原始 ID，保持上下游 ID 配对语义。
...
/// 将 DNS 查询报文的 ID 重写为 0（RFC 9250 §4.2 强制要求）。
///
/// 返回 (重写后的报文, 原始 ID)，供 caller 在响应上恢复 ID。
/// 对齐 sing-box `quic.go:159` 的 `WriteMessage(stream, 0, message)` 行为：
/// sing-box 在 `WriteMessage` 内部将 `exMessage.Id = messageId`（这里为 0）。
...
/// 将响应的 ID 还原为原始查询 ID（`set_doq_query_id_zero` 的逆操作）。
/// 对齐 sing-box `client.go:307` 的 `response.Id = messageId` 行为。
...
async fn doq_query_once(
    ...
    send.write_all(&len.to_be_bytes())
        .await
        .map_err(|e| anyhow::anyhow!("DoQ stream write (len) failed: {e}"))?;
    send.write_all(msg)
        .await
        .map_err(|e| anyhow::anyhow!("DoQ stream write (msg) failed: {e}"))?;
