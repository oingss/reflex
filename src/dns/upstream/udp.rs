//! UDP 协议实现：UdpState 复用收发状态、udp_query、queryId 重写、EDNS OPT 解析、label 压缩。
...
/// 直连 UDP 上游的复用状态：包含 socket、后台 recv_loop、callbacks 分发表。
///
/// 对齐 sing-box `dns/transport/udp.go` 的设计：
/// - 单 socket 长期复用，所有并发查询共享
/// - 后台 `recv_loop` 任务持续读取响应，按 queryId 分发给对应 caller
/// - caller 通过 `oneshot` channel 接收自己的响应
/// - 避免旧实现「多 caller 各自 recv_from 互相抢响应」的正确性 bug
///
/// 同时支持动态 EDNS UDPSize 跟踪：当请求 OPT 记录的 UDPSize 超过当前值时，
/// CAS 更新并触发 socket 重建（丢弃旧 socket、让 recv_loop 退出、下次查询重建）。
pub struct UdpState {
...
impl UdpState {
    ...
    /// 触发 socket 重建：让 recv_loop 退出，外部状态置 None，下次查询重建。
    /// 对齐 sing-box 在 EDNS UDPSize 变化时的 `t.Reset()`。
    /// 调用方在持有 `udp_state_v4/v6` Mutex 的情况下将状态置 None 即可，
    /// 这里只负责让 recv_loop 自然退出（通过 close socket）。
    pub(super) fn invalidate(self: &Arc<Self>) {
        // 标记 recv_loop 退出中，避免 ensure_recv_loop 重复 spawn
        self.recv_loop_started.store(0, Ordering::SeqCst);
        // socket drop 会让 recv_from 返回错误，recv_loop 自然退出
        // 注意：这里不显式 drop socket，因为 Arc 可能有其他引用；
        // 实际靠外部将 udp_state_v4/v6 置 None 来释放最后一个引用
    }
}
...
/// RAII guard：确保 callback 在 query future 被 drop 时（如外层 timeout 触发）
/// 仍能从 `callbacks` map 中清理。
///
/// **修复 callback 泄漏 bug**：
/// 旧实现在外层 `timeout()` 触发时直接 drop `udp_query_with_state` 未来，
/// `rx`（`oneshot::Receiver`）被 drop，但 `callbacks` map 中的
/// `oneshot::Sender` 未被清理，导致：
/// 1. queryId 空间被占用（`next_available_query_id` 跳过这些 id）
/// 2. `oneshot::Sender` 持有内存不被释放
/// 3. 在持续丢包 / 服务器无响应场景下累积，最终耗尽 queryId 空间
///
/// 对齐 sing-box `udp.go:189-193` 的 `defer { delete(t.callbacks, queryId) }`：
/// sing-box 在函数退出时（包括 `ctx.Done()` 超时）通过 defer 删除 callback。
/// Rust 没有 defer，用 RAII guard 达到同样效果。
...
pub(super) async fn udp_query_with_state(
    ...
    // ── queryId 重写（对齐 sing-box dns/transport/udp.go:200-201）─────────────
    // 用内部分配的 queryId 替换 caller 原始的 queryId，避免多 caller 并发使用
    // 同一 queryId 时响应被错误分发（罕见但可能，比如多个 caller 都从 0 开始）。
    //
    // 流程：
    //   1. 分配内部 queryId
    //   2. 复制 msg 并把前 2 字节重写为内部 queryId
    //   3. 注册 callback，拿到 Receiver
    //   4. 压缩请求 + 发送
    //   5. 等待 Receiver，超时则注销 callback
    //   6. 收到响应后把 queryId 还原为原始值再返回（让 caller 看到自己发出的 id）
    //
    // 若 msg 异常短无法提取 queryId，回退到一次性 socket 查询（不进 callbacks 分发）。
    if msg.len() < 12 {
    ...
    // ── 压缩请求（对齐 sing-box dns/transport/udp.go:199 exMessage.Compress = true）
    // DNS label 指针压缩可减小请求字节数（典型场景省 30%+）。
    // reflex 当前不引入完整 DNS Message 库，压缩暂以「就地 label 指针压缩」实现：
    // 在原报文基础上对 QNAME 之外重复出现的 label 用指针替换。
    // 简化处理：先尝试调用 compress_dns_labels；失败回退到原始报文（不影响正确性）。
    ...
    // Truncated 检查（对齐 sing-box udp.go:121-125 的 response.Truncated）
    // 仍用响应的 flags 字节判定 TC 位（与原实现一致），并自动重试 TCP
...
/// 从 DNS 请求报文中提取 EDNS OPT 记录的 UDPSize（如有）。
/// 对齐 sing-box `message.IsEdns0().UDPSize()`。
///
/// EDNS OPT 记录位于 Additional 段，TYPE=41。这里做最小解析：
/// 只读 header 的 ARCOUNT，再扫描 Additional 段找 TYPE=41 的记录，
/// 取其 CLASS 字段（即 UDPSize）。
///
/// 解析失败返回 0（视为无 EDNS OPT）。
...
/// DNS 报文压缩（label pointer compression）。
///
/// 对齐 sing-box dns/transport/udp.go:199 `exMessage.Compress = true`。
/// 通过 RFC 1035 label pointer（前两 bit 11）压缩 QNAME 中重复的 label，
/// 减小请求字节数。
///
/// 实现策略：手写最小压缩器，对报文里出现的每个 label 记录其首次出现偏移，
/// 后续出现相同 label 时用 2 字节指针替换。仅压缩 Question 段的 QNAME
/// （Answer/Authority/Additional 段在请求中通常不存在）。
///
/// 压缩失败时返回 Err，调用方回退到原始未压缩报文（不影响正确性，仅多占带宽）。
