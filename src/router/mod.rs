...
pub enum RouteAction {
    ...
    /// - `force_domain`：嗅探白名单（仅这些域名做嗅探，空 = 不限制）
    /// - `skip_domain`：嗅探黑名单（这些域名跳过嗅探）
    /// - `skip_src_address`：源 IP CIDR 黑名单（这些来源跳过嗅探）
    Sniff {
        timeout_ms: u64,
...
impl Router {
    ...
    pub fn from_config(
        ...
        let has_process_rules = config.rules.iter().any(|r| {
            !r.process_name.is_empty() || !r.process_path.is_empty()
        });
    ...
    fn matches(
        ...
        if has_addr_rules || has_private_ip {
            ...
            let ruleset_ok = !has_ruleset
                || self.match_rulesets(sniffed_domain, target, resolved_ip, port_val);
            ...
            let private_ip_ok =
                !has_private_ip || match_private_ip(target, resolved_ip);
    ...
    fn match_rulesets(
        ...
        for rs in &self.rulesets {
            ...
            if let Some(d) = sniffed_domain {
                if rs.matches(&MatchTarget::Domain(d)) {
                    return true;
                }
            }
            if let Target::Domain(h, _) = target {
                if rs.matches(&MatchTarget::Domain(h)) {
                    return true;
                }
            }
...
mod tests {
    ...
    fn route<'a>(
        ...
        r.route(inbound, Some(net), target, None, None, None, None, None).0
    }
    ...
    fn source_ip_cidr_match() {
        ...
        // 无来源 IP 的连接不命中
        assert_eq!(
            r.route("in", Some(NetworkKind::Tcp), &t, None, None, None, None, None).0,
            &RouteAction::Outbound("proxy".into())
        );
    }
    ...
    fn dns_out_action() {
        ...
        assert_eq!(
            r.route("dns-in", Some(NetworkKind::Udp), &t, None, None, None, None, None).0,
            &RouteAction::DnsOut
        );
    }
    ...
    fn override_address_and_port_carried_in_route_options() {
        ...
        let (action, _rt, _rp, opts) = r.route("in", Some(NetworkKind::Tcp), &t, None, None, None, None, None);
        assert_eq!(action, &RouteAction::Outbound("direct".into()));
    ...
    fn udp_timeout_carried_in_route_options() {
        ...
        let (_action, _rt, _rp, opts) = r.route("in", Some(NetworkKind::Udp), &t, None, None, None, None, None);
    ...
    fn final_action_has_empty_route_options() {
        ...
        let (_action, rt, _rp, opts) = r.route("in", Some(NetworkKind::Tcp), &t, None, None, None, None, None);
        assert_eq!(rt, "final");
...
mod hijack_dns_tests {
    ...
    fn hijack_dns_with_protocol_dns_action() {
        ...
        assert_eq!(
            router
                .route("any-in", Some(NetworkKind::Udp), &t, Some("dns"), None, None, None, None)
                .0,
            &RouteAction::DnsOut
        );
    }
    ...
    fn hijack_dns_with_inbound_action() {
        ...
        assert_eq!(
            router
                .route("dns-in", Some(NetworkKind::Udp), &t, None, None, None, None, None)
                .0,
            &RouteAction::DnsOut
        );
    }
