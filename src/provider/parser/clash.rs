...
use crate::config::outbound::{
    AnyTlsOutboundConfig, HttpHeadersConfig, HttpOutboundConfig, Hysteria2OutboundConfig,
    OutboundConfig, RealityConfig, ShadowsocksOutboundConfig, SocksOutboundConfig, TlsConfig,
    ...
    WireGuardOutboundConfig, WsTransportConfig,
};
...
/// HTTP(S) 代理，Clash type 为 http。`tls: true` 时视为 HTTPS 代理
/// （先 TLS 握手再发 CONNECT），`sni` 映射为 TLS server_name，
/// `skip-cert-verify` 映射为 TLS insecure。
fn build_http(tag: String, p: ClashProxy) -> anyhow::Result<OutboundConfig> {
    let tls_enabled = p.tls.unwrap_or(false);
    Ok(OutboundConfig::Http(HttpOutboundConfig {
        tag,
        server: p.server,
        server_port: p.port,
        username: p.username,
        password: p.password,
        path: None,
        headers: HttpHeadersConfig::default(),
        tls: TlsConfig {
            enabled: tls_enabled,
            server_name: p.sni,
            insecure: p.skip_cert_verify,
