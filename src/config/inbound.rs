...
impl MixedInboundConfig {
    ...
    pub fn udp_timeout_duration(&self) -> Option<std::time::Duration> {
        self.udp_timeout.as_deref().and_then(|s| {
            match crate::config::outbound::parse_duration(s) {
                Ok(d) => Some(d),
                Err(e) => {
                    warn!(
                        tag = %self.tag,
...
mod tests {
    ...
    fn socks_and_mixed_udp_timeout_duration() {
        ...
        if let InboundConfig::Mixed(c) = &ib {
            ...
            // 未配置时为 None
            assert!(MixedInboundConfig::udp_timeout_duration(
                &MixedInboundConfig {
                    tag: "x".into(),
    ...
    fn parse_tun_full() {
        ...
        if let InboundConfig::Tun(c) = &ib {
            ...
            assert_eq!(c.loopback_address, vec!["127.0.0.1".to_string(), "::1".to_string()]);
