...
pub enum InboundConfig {
    ...
    /// HTTP 服务端入站（移植自 flux，支持可选 TLS 包装为 HTTPS 代理）
    #[serde(rename = "http-server")]
    HttpServer(HttpServerInboundConfig),
    /// SOCKS5 服务端入站（移植自 flux，仅 CONNECT，不支持 UDP ASSOCIATE）
    #[serde(rename = "socks-server")]
    SocksServer(SocksServerInboundConfig),
...
impl InboundConfig {
    pub fn tag(&self) -> &str {
        match self {
            ...
            Self::Http(c) => &c.tag,
            Self::Socks(c) => &c.tag,
            Self::HttpServer(c) => &c.tag,
            Self::SocksServer(c) => &c.tag,
            Self::Vless(c) => &c.tag,
            Self::Vmess(c) => &c.tag,
            Self::Trojan(c) => &c.tag,
    ...
    pub fn listen_addr(&self) -> (&str, u16) {
        match self {
            ...
            Self::HttpServer(c) => (&c.listen, c.listen_port),
            Self::SocksServer(c) => (&c.listen, c.listen_port),
...
// ── HTTP Server（移植自 flux 的服务端 HTTP 代理入站）────────────────────────

/// HTTP 服务端入站配置（`type: "http-server"`，移植自 flux）。
///
/// 与客户端本地用途的 [`HttpInboundConfig`] 相比，面向公网服务端场景：
/// - 可选 TLS 包装（`tls.enabled` 时为 HTTPS 代理，客户端需支持 HTTPS 代理）
/// - 转发普通 HTTP 请求时剥离 `Proxy-Authorization` / `Proxy-Connection` 头
///
/// 配置写法（对齐 flux）：
/// ```json
/// {
...
/// ```
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct HttpServerInboundConfig {
    pub tag: String,
    ...
    /// 可选 TLS（disabled 时为明文 HTTP 代理）。
    #[serde(default)]
    pub tls: InboundTlsConfig,
}

// ── SOCKS Server（移植自 flux 的服务端 SOCKS5 入站）─────────────────────────

/// SOCKS5 服务端入站配置（`type: "socks-server"`，移植自 flux）。
///
/// 与 [`SocksInboundConfig`] 的区别：纯服务端 CONNECT 代理，
/// **不支持 UDP ASSOCIATE**（与 flux 行为一致）。
///
/// 配置写法：
/// ```json
/// {
...
/// ```
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct SocksServerInboundConfig {
    pub tag: String,
...
mod tests {
    ...
    fn parse_http_server_inbound() {
        ...
        let ib: InboundConfig = serde_json::from_value(v).unwrap();
        assert_eq!(ib.tag(), "http-srv");
        assert!(matches!(ib, InboundConfig::HttpServer(_)));
        if let InboundConfig::HttpServer(c) = &ib {
            assert_eq!(c.listen, "0.0.0.0");
    ...
    fn parse_http_server_defaults() {
        ...
        if let InboundConfig::HttpServer(c) = &ib {
            ...
            panic!("expected HttpServer");
        }
    }

    #[test]
    fn parse_socks_server_inbound() {
        let v = json!({
            "type": "socks-server",
            "tag": "socks-srv",
            "listen": "0.0.0.0",
        ...
        let ib: InboundConfig = serde_json::from_value(v).unwrap();
        assert_eq!(ib.tag(), "socks-srv");
        assert!(matches!(ib, InboundConfig::SocksServer(_)));
        if let InboundConfig::SocksServer(c) = &ib {
            assert_eq!(c.listen_port, 1080);
            assert_eq!(c.users.len(), 1);
        }
    }
    ...
    #[test]
    fn parse_socks_server_defaults() {
        let v = json!({
            "type": "socks-server",
            "tag": "socks-srv",
            "listen_port": 1080
