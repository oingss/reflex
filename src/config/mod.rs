...
impl Config {
    ...
    pub fn from_file(path: impl AsRef<Path>) -> anyhow::Result<Self> {
        ...
        Self::from_text_with_format(&raw, fmt).with_context(|| {
            format!("failed to parse config file ({fmt}): {}", path.display())
        })
    }
...
"#;
    ...
    fn format_from_path_extension() {
        ...
        assert_eq!(ConfigFormat::from_path(Path::new("a.txt")), ConfigFormat::Json);
    }
