...
impl Config {
    ...
    fn validate(&self) -> anyhow::Result<()> {
        ...
        for (i, rule) in self.route.rules.iter().enumerate() {
            // 非 route 动作（hijack-dns/sniff/resolve/block/reject）或 dns-out 无需校验。
            // requires_outbound_tag() 统一判断：仅 route 动作 + outbound 非空 + 非 dns-out 才需校验。
            let tag = rule.outbound_tag();
            if rule.requires_outbound_tag() && !ob_tags.contains(tag) {
                anyhow::bail!("route.rules[{i}].outbound '{tag}' does not match any outbound tag");
            }
        }
        ...
        for (i, rule) in self.dns.rules.iter().enumerate() {
            // 校验规则引用的 server tag 都存在（route 动作；block 动作无需 server）
            for tag in rule.server_tags() {
                if !dns_tags.contains(tag.as_str()) {
                    anyhow::bail!(
