...
pub struct DirectOutboundConfig {
    ...
    /// 域名解析偏好，对齐 sing-box `domain_strategy`：
    /// - `prefer_ipv4`（默认）：优先 IPv4，回退 IPv6
    /// - `ipv4_only` / `ipv6_only`：仅使用对应协议栈
    /// - `prefer_ipv6`：优先 IPv6，回退 IPv4
    ///
    /// 仅对域名目标生效；同时在 happy_eyeballs 候选排序前应用（过滤/重排）。
    #[serde(default)]
    pub domain_strategy: Option<crate::config::dns::ResolveStrategy>,

    /// TCP 连接超时（毫秒），对齐 sing-box `connect_timeout`。默认 5000。
    #[serde(default)]
...
/// HTTP 代理出站配置（与 sing-box `HTTPOutboundOptions` / sing `protocol/http.Client` 对齐）。
///
/// 通过 HTTP CONNECT 方法建立 TCP 隧道。仅支持 TCP：`connect_udp` 不实现，
/// `handle_udp` 直接报错，等价于 sing 官方实现中 `ListenPacket` 返回
/// `os.ErrInvalid`（HTTP/1.1 CONNECT 本身不承载 UDP）。
///
/// 配置示例：
/// ```json
/// {
...
pub struct HttpOutboundConfig {
    ...
    /// CONNECT 请求路径（部分反代网关需要，一般不需要设置）。
    /// 与 `headers.Host` 互斥——sing 官方实现中二者同时提供会直接报错，
    /// 这里保持同样的语义（见 http.rs `connect_tunnel` 的检查）。
    #[serde(default)]
    pub path: Option<String>,

    /// 附加的自定义请求头（随 CONNECT 请求一起发送）。
    /// 若包含 `Host` 键，会被单独取出作为请求行 Host 使用，
    /// 而不进入普通头部列表——对齐 sing `NewClient` 中 `headers.Get("Host")`
    /// + `headers.Del("Host")` 的处理方式。
    #[serde(default)]
    pub headers: HttpHeadersConfig,

    /// TLS 配置（启用后为 HTTPS 代理，先建立 TLS 再发 CONNECT）
    #[serde(default)]
    pub tls: TlsConfig,
}

/// HTTP 出站自定义请求头。既支持 `{"Header": "value"}` 单值写法，
/// 也支持 `{"Header": ["v1", "v2"]}` 多值写法（与 sing-box `Listable[string]` 对齐）。
///
/// 注：不能用 `#[serde(transparent)]`——transparent 要求内部字段不带
/// `#[serde(default)]`（二者冲突，字段级默认值在“透明”序列化下没有意义）。
/// 改用 `#[serde(from)]`/`#[serde(into)]` 也没必要，直接手写
/// `Deserialize`/`Serialize` 委托给内部 HashMap 即可，效果与 transparent
/// 完全等价（JSON 层面仍然就是一个普通对象 `{"Header": ...}`）。
#[derive(Debug, Clone, Default)]
pub struct HttpHeadersConfig(pub std::collections::HashMap<String, HttpHeaderValue>);

impl Serialize for HttpHeadersConfig {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: serde::Serializer,
    {
...
impl<'de> Deserialize<'de> for HttpHeadersConfig {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: serde::Deserializer<'de>,
    {
        let map = std::collections::HashMap::<String, HttpHeaderValue>::deserialize(deserializer)?;
        Ok(HttpHeadersConfig(map))
    }
}
...
impl HttpOutboundConfig {
    /// 取出 `Host` 头（大小写不敏感），返回 `(host, 其余头部列表)`。
    /// 对齐 sing `NewClient`：headers 中的 Host 会被摘出用作请求 Host，
    /// 不会作为普通头部再发送一次。多值 header 展开为多行 `(name, value)`。
    pub fn take_host_and_headers(&self) -> (Option<String>, Vec<(String, String)>) {
        let mut host = None;
        let mut out = Vec::new();
        for (k, v) in &self.headers.0 {
            ...
            match v {
                HttpHeaderValue::Single(s) => out.push((k.clone(), s.clone())),
                HttpHeaderValue::Multiple(list) => {
                    for s in list {
